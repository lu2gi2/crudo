...
This milestone adds a deterministic Anthropic-compatible mock service plus a reproducible CLI harness for the Rust `crudo` binary.

## Artifacts

- `crates/mock-anthropic-service/` — mock `/v1/messages` service
- `crates/crudo-cli/tests/mock_parity_harness.rs` — end-to-end clean-environment harness
...
Scenario-to-PARITY mappings live in `mock_parity_scenarios.json`; keep this manifest aligned with `rust/crates/crudo-cli/tests/mock_parity_harness.rs` and `PARITY.md` via `python3 scripts/run_mock_parity_diff.py --no-run`.

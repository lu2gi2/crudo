# 🦞 Crudo — Rust Implementation

A high-performance Rust rewrite of the Crudo CLI agent harness. Built for speed, safety, and native tool execution.
...
cd rust/
cargo run -p crudo-cli -- --help

# Build the workspace
cargo build --workspace

# Run the interactive REPL
cargo run -p crudo-cli -- --model claude-opus-4-7

# One-shot prompt
cargo run -p crudo-cli -- prompt "explain this codebase"

# JSON output for automation
cargo run -p crudo-cli -- --output-format json prompt "summarize src/main.rs"
```
...
- `crates/crudo-cli/tests/mock_parity_harness.rs` — clean-env CLI harness
...
| Todo tracking | ✅ |
| Notebook editing | ✅ |
| CLAUDE.md / CRUDO.md / AGENTS.md project memory | ✅ |
| Config file hierarchy (`.crudo.json` + merged config sections) | ✅ |
| Permission system | ✅ |
...
```text
crudo [OPTIONS] [COMMAND]

Flags:
  --model MODEL
  --output-format text|json  (case-insensitive; CRUDO_OUTPUT_FORMAT supplies the default, flags override env)
...
`crudo acp` is a local discoverability surface for editor-first users: it reports the current ACP/Zed status without starting the runtime. As of April 16, 2026, crudo does **not** ship an ACP/Zed daemon or JSON-RPC entrypoint yet, and `crudo acp serve` is only a status alias until the real protocol surface lands. Status queries exit 0 and expose the same machine-readable contract via `--output-format json`; malformed ACP invocations exit 1 with `kind: unsupported_acp_invocation`.
`--output-format` accepts `text` or `json` in any casing. `CRUDO_OUTPUT_FORMAT=json` selects JSON as the default for non-interactive commands, explicit flags override it, repeated flags warn on stderr, and status JSON exposes `format_source`, `format_raw`, and `format_overridden`. Help and doctor output also surface `CRUDO_LOG` / `RUST_LOG` as the logging environment knobs.
`crudo version --output-format json` is the provenance probe for automation: it reports full `git_sha`, derived `git_sha_short`, `is_dirty`, `branch`, `commit_date`, `commit_timestamp`, `rustc_version`, runtime `executable_path`, and `binary_provenance`; the text report is available as `human_readable` instead of a duplicate `message` field.
`status --output-format json` reports loaded project memory files under `workspace.memory_files[]` with each file's `path`, `source` (`claude_md`, `crudo_md`, `agents_md`, or scoped/rule sources), `origin`, `scope_path`, `outside_project`, `chars`, and `contributes`; `crudo doctor --output-format json` includes a dedicated `memory` check. Root instruction-file priority is `CLAUDE.md`, then `CRUDO.md`, then `AGENTS.md`, discovery is bounded to the current git root when present (otherwise cwd only), and all non-duplicate loaded files contribute to the rendered system prompt.
`crudo mcp --output-format json` reports partial MCP config success: valid servers remain in `servers[]` while malformed siblings appear in `invalid_servers[]`, with `total_configured`, `valid_count`, and `invalid_count` split out for automation. `status` mirrors this as `mcp_validation`, and doctor includes an `mcp validation` check.
...
Shorthand prompt mode honors the POSIX `--` end-of-flags separator, so `crudo -- "-prompt-with-dash"` and unknown dash-prefixed non-flag text stay on the prompt path instead of being treated as CLI options.
`crudo dump-manifests` is self-contained: it emits the Rust resolver inventory for the selected workspace (commands, tools, agents, skills, and bootstrap phases) without requiring an upstream Claude Code TypeScript checkout. Use `--manifests-dir PATH` only to scope resolver discovery to another directory.
...
Notable crudo-first surfaces now available directly in slash form:
...
See [`../USAGE.md`](../USAGE.md) for usage examples and run `cargo run -p crudo-cli -- --help` for the live canonical command list.
...
└── crates/
    ...
    ├── crudo-cli/   # Main CLI binary (`crudo`)
...
- **crudo-cli** — REPL, one-shot prompt, direct CLI subcommands, streaming display, tool call rendering, CLI argument parsing

...
impl DefinitionSource {
    fn report_scope(self) -> DefinitionScope {
        match self {
            Self::ProjectCrudo | Self::ProjectCodex | Self::ProjectClaude => {
                DefinitionScope::Project
            }
            Self::UserCrudoConfigHome | Self::UserCodexHome => DefinitionScope::UserConfigHome,
            Self::UserCrudo | Self::UserCodex | Self::UserClaude => DefinitionScope::UserHome,
        }
    }
        ...
        Some(args) if args.starts_with("list ") => {
            ...
            if filter.starts_with('-') {
                return Ok(serde_json::json!({
                    ...
                    "unexpected": filter,
                    "hint": "Usage: crudo agents list [<filter>]\nFilters are name substrings, not flags.",
                }));
            }
            let roots = discover_definition_roots(cwd, "agents");
            let collection = load_agents_from_roots_with_invalids(&roots)?;
                ...
                || args.starts_with("describe ") =>
        {
            ...
            if let Some(extra_token) = extra {
                return Ok(serde_json::json!({
                    ...
                    "hint": format!("Usage: crudo agents show <name>\nUnexpected extra: '{extra_token}'"),
                }));
            }
            let roots = discover_definition_roots(cwd, "agents");
            let collection = load_agents_from_roots_with_invalids(&roots)?;
            ...
            if matched.is_empty() {
                return Ok(serde_json::json!({
                    ...
                    "hint": "Run `crudo agents list` to see available agents.",
                }));
            }
        ...
        Some(args) if args.starts_with("create ") => {
            ...
            if let Some(extra) = parts.next() {
                return Ok(json!({
                    ...
                    "error_kind": "unexpected_extra_args",
                    "unexpected": extra,
                    "hint": format!("Usage: crudo agents create <name>\nUnexpected extra: '{extra}'"),
                }));
            }
    ...
    match normalize_optional_args(args) {
                ...
                || args.starts_with("describe ") =>
        {
            ...
            if matched.is_empty() {
                return Err(std::io::Error::new(
                    std::io::ErrorKind::NotFound,
                    format!("skill '{name_raw}' not found\nRun `crudo skills list` to see available skills."),
                ));
            }
        ...
        Some(args) if args.starts_with("install ") => {
            ...
            if target.is_empty() {
                return Err(std::io::Error::new(
                    std::io::ErrorKind::InvalidInput,
                    "missing_argument: skills install requires an install source.\nUsage: crudo skills install [--project] <path>",
                ));
            }
            let install = if project_flag {
                let project_root = cwd.join(".crudo").join("skills");
        ...
        Some(args) if args.starts_with("list ") => {
            ...
            if filter.starts_with('-') {
                return Ok(serde_json::json!({
                    ...
                    "error_kind": "unknown_option",
                    "unexpected": filter,
                    "hint": "Usage: crudo skills list [<filter>]\nFilters are name substrings, not flags.",
                }));
            }
            let roots = discover_skill_roots(cwd);
            let collection = load_skills_from_roots_with_drift(&roots)?;
                ...
                || args.starts_with("describe ") =>
        {
            ...
            if let Some(extra_token) = extra {
                return Ok(json!({
                    ...
                    "error_kind": "unexpected_extra_args",
                    "unexpected": extra_token,
                    "hint": format!("Usage: crudo skills show <name>\nUnexpected extra: '{extra_token}'"),
                }));
            }
            let roots = discover_skill_roots(cwd);
            let collection = load_skills_from_roots_with_drift(&roots)?;
            ...
            if matched.is_empty() {
                return Ok(json!({
                    ...
                    "hint": "Run `crudo skills list` to see available skills.",
                }));
            }
    ...
    match normalize_optional_args(args) {
        ...
        Some(args) if args.split_whitespace().next() == Some("list") && args.contains(' ') => {
            ...
            Ok(render_mcp_unsupported_action_text(
                args,
                "list accepts no filter argument; use `crudo mcp list`",
            ))
        }
        Some(args) if matches!(args.split_whitespace().next(), Some("info" | "describe")) => {
            Ok(render_mcp_unsupported_action_text(
                args,
                "use `crudo mcp show <server>` to inspect a server",
            ))
        }
...
fn render_mcp_unsupported_action_json(action: &str, hint: &str) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "crudo mcp [list|show <server>|help]",
        },
    })
}
...
fn discover_definition_roots(cwd: &Path, leaf: &str) -> Vec<(DefinitionSource, PathBuf)> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_root(
            &mut roots,
            DefinitionSource::ProjectCrudo,
            ancestor.join(".crudo").join(leaf),
        );
        push_unique_root(
            &mut roots,
    ...
    if let Ok(crudo_config_home) = env::var("CRUDO_CONFIG_HOME") {
        push_unique_root(
            &mut roots,
            DefinitionSource::UserCrudoConfigHome,
            PathBuf::from(crudo_config_home).join(leaf),
        );
    }
...
fn discover_skill_roots(cwd: &Path) -> Vec<SkillRoot> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectCrudo,
            ancestor.join(".crudo").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectCrudo,
            ancestor.join(".crudo").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
        push_unique_skill_root(
            &mut roots,
    ...
    if let Ok(crudo_config_home) = env::var("CRUDO_CONFIG_HOME") {
        let crudo_config_home = PathBuf::from(crudo_config_home);
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserCrudoConfigHome,
            crudo_config_home.join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserCrudoConfigHome,
            crudo_config_home.join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
...
fn create_agent(name: &str, cwd: &Path) -> std::io::Result<CreatedAgent> {
    ...
    let root = cwd.join(".crudo").join("agents");
...
fn default_skill_install_root() -> std::io::Result<PathBuf> {
    if let Ok(crudo_config_home) = env::var("CRUDO_CONFIG_HOME") {
        return Ok(PathBuf::from(crudo_config_home).join("skills"));
    }
    if let Ok(codex_home) = env::var("CODEX_HOME") {
        return Ok(PathBuf::from(codex_home).join("skills"));
    }
    if let Some(home) = env::var_os("HOME") {
        return Ok(PathBuf::from(home).join(".crudo").join("skills"));
    }
    Err(std::io::Error::new(
        std::io::ErrorKind::NotFound,
        "unable to resolve a skills install root; set CRUDO_CONFIG_HOME or HOME",
    ))
}
...
fn render_agents_missing_argument_json(action: &str, argument: &str) -> Value {
    json!({
        ...
        "argument": argument,
        "hint": "Usage: crudo agents create <name>",
    })
}
...
fn render_agent_create_error_json(name: &str, error: &std::io::Error) -> Value {
    ...
    json!({
        ...
        "name": name,
        "message": message,
        "hint": "Use `crudo agents create <name>` with a simple alphanumeric, dash, underscore, or dot name.",
    })
}
...
fn render_mcp_server_report_json(
    ...
    match mcp.get(server_name) {
        ...
        None => json!({
            ...
            "hint": "Run `crudo mcp list` to see configured servers.",
            "total_configured": mcp.total_configured(),
            "valid_count": mcp.valid_count(),
            "invalid_count": mcp.invalid_count(),
            "invalid_servers": invalid_mcp_servers_json(mcp.invalid_servers()),
...
fn render_agents_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       crudo agents [list|show <name>|create <name>|help]".to_string(),
        "  Format           TOML files (.toml); create scaffolds .crudo/agents/<name>.toml"
            .to_string(),
        "  Sources          .crudo/agents, ~/.crudo/agents, $CRUDO_CONFIG_HOME/agents".to_string(),
    ];
...
fn render_agents_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "crudo agents [list|show <name>|create <name>|help]",
            "format": "toml",
            "create": "crudo agents create <name>",
            "sources": [".crudo/agents", "~/.crudo/agents", "~/.codex/agents", "$CRUDO_CONFIG_HOME/agents"],
        },
...
fn render_skills_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       crudo skills [list|show <name>|install [--project] <path>|uninstall <name>|help|<skill> [args]]".to_string(),
        "  Lifecycle        install <path>, uninstall <name>".to_string(),
        "  Invoke           /skills help overview -> $help overview".to_string(),
        "  Install root     $CRUDO_CONFIG_HOME/skills or ~/.crudo/skills (use --project for .crudo/skills)".to_string(),
        "  Sources          .crudo/skills, .omc/skills, .agents/skills, .codex/skills, .claude/skills, ~/.crudo/skills, ~/.omc/skills, ~/.claude/skills/omc-learned, ~/.codex/skills, ~/.claude/skills, legacy /commands".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_skills_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "aliases": ["/skill"],
            "direct_cli": "crudo skills [list|show <name>|install <path>|uninstall <name>|help|<skill> [args]]",
            "lifecycle": ["install <path>", "uninstall <name>"],
            "invoke": "/skills help overview -> $help overview",
            "install_root": "$CRUDO_CONFIG_HOME/skills or ~/.crudo/skills",
            "sources": [
                ".crudo/skills",
                ".omc/skills",
                ".agents/skills",
...
fn render_mcp_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       crudo mcp [list|show <server>|help]".to_string(),
        "  Sources          .crudo/settings.json, .crudo/settings.local.json".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_mcp_missing_argument_text(action: &str) -> String {
    let hint = match action {
        "show" => "use `crudo mcp show <server>` to inspect a server",
...
fn render_mcp_missing_argument_json(action: &str) -> Value {
    let (message, hint) = match action {
        "show" => (
            ...
            "Usage: crudo mcp show <server>",
        ),
        _ => (
            "mcp action requires an argument",
            "Usage: crudo mcp [list|show <server>|help]",
        ),
    };
    json!({
        "kind": "mcp",
        ...
        "usage": {
            "slash_command": "/mcp [list|show <server>|help]",
            "direct_cli": "crudo mcp [list|show <server>|help]",
            "sources": [".crudo/settings.json", ".crudo/settings.local.json"],
        },
...
fn render_mcp_usage_json(unexpected: Option<&str>) -> Value {
    ...
    json!({
        ...
        "usage": {
            "slash_command": "/mcp [list|show <server>|help]",
            "direct_cli": "crudo mcp [list|show <server>|help]",
            "sources": [".crudo.json", ".crudo/settings.json", ".crudo/settings.local.json"],
        },
        "unexpected": unexpected,
    })
}
...
fn definition_source_id(source: DefinitionSource) -> &'static str {
    match source {
        DefinitionSource::ProjectCrudo
        | DefinitionSource::ProjectCodex
        | DefinitionSource::ProjectClaude => "project_crudo",
...
mod tests {
    ...
    fn skills_show_and_list_filter_do_not_invoke_model() {
        ...
        // Regression for: `crudo skills show plan` unexpectedly spawned a model session.
    ...
    fn renders_per_command_help_detail() {
        ...
        assert!(help.contains("Summary          Manage Crudo plugins"));
        assert!(help.contains("Aliases          /plugins, /marketplace"));
    ...
    fn renders_agents_reports_as_json() {
        ...
        let original_crudo_config_home = std::env::var_os("CRUDO_CONFIG_HOME");
        ...
        assert_eq!(report["agents"][2]["shadowed_by"]["id"], "project_crudo");
        ...
        assert_eq!(
            ...
            "crudo agents [list|show <name>|create <name>|help]"
        );
        ...
        restore_env_var("CRUDO_CONFIG_HOME", original_crudo_config_home);
    ...
    fn resolves_project_skills_and_legacy_commands_from_shared_registry() {
        ...
        let project_skills = workspace.join(".crudo").join("skills");
        let legacy_commands = workspace.join(".crudo").join("commands");
    ...
    fn renders_skills_reports_as_json() {
        ...
        assert_eq!(report["skills"][0]["source"]["id"], "project_crudo");
        ...
        assert_eq!(report["skills"][1]["source"]["id"], "project_crudo");
        ...
        assert_eq!(report["skills"][3]["shadowed_by"]["id"], "project_crudo");
        ...
        assert_eq!(
            help["usage"]["direct_cli"],
            "crudo skills [list|show <name>|install <path>|uninstall <name>|help|<skill> [args]]"
        );
    ...
    fn agents_and_skills_usage_support_help_and_unexpected_args() {
        ...
        assert!(agents_help
            .contains("Direct CLI       crudo agents [list|show <name>|create <name>|help]"));
        ...
        assert!(skills_help.contains("Install root     $CRUDO_CONFIG_HOME/skills or ~/.crudo/skills (use --project for .crudo/skills)"));
    ...
    fn mcp_usage_supports_help_and_unexpected_args() {
        ...
        assert!(help.contains("Direct CLI       crudo mcp [list|show <server>|help]"));
    ...
    fn renders_mcp_reports_from_loaded_config() {
        ...
        fs::create_dir_all(workspace.join(".crudo")).expect("workspace config dir");
        fs::create_dir_all(&config_home).expect("config home");
        fs::write(
            workspace.join(".crudo").join("settings.json"),
            r#"{
              "mcpServers": {
                "alpha": {
                  "command": "uvx",
        ...
        fs::write(
            workspace.join(".crudo").join("settings.local.json"),
            r#"{
              "mcpServers": {
                "remote": {
                  "type": "ws",
    ...
    fn renders_mcp_reports_as_json() {
        ...
        assert_eq!(help["action"], "help");
        assert_eq!(help["usage"]["sources"][0], ".crudo.json");
    ...
    fn mcp_loads_valid_servers_and_reports_invalid_siblings_440() {
        ...
        fs::create_dir_all(workspace.join(".crudo")).expect("create workspace .crudo dir");

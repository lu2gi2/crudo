# AGENTS.md — crudo-cli (the `crudo` binary)

## OVERVIEW

Crate `crudo-cli` produces the `crudo` binary (~25 subcommands). Wires together api, runtime, tools, commands, plugins; terminal via crossterm/rustyline/syntect. Hand-rolled arg parser (no clap).
...
- Package/bin name mismatch: crate is `crudo-cli`, binary is `crudo`. Watch for this in paths and test macros.

## TESTS

All tests live in `tests/` (6 files), black-box style. They spawn `env!("CARGO_BIN_EXE_crudo")` in unique temp dirs (AtomicU64 counter).

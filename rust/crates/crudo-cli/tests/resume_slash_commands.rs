...
fn resumed_binary_accepts_slash_commands_with_arguments() {
    ...
    assert!(stdout.contains("Resume previous  crudo --resume"));
...
fn resumed_config_command_loads_settings_files_end_to_end() {
    ...
    fs::create_dir_all(project_dir.join(".crudo")).expect("project config dir should exist");
    ...
    fs::write(
        project_dir.join(".crudo").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
    .expect("local config should write");

    // when
    let output = run_crudo_with_env(
        &project_dir,
        &[
            "--resume",
            session_path.to_str().expect("utf8 path"),
...
fn resume_latest_restores_the_most_recent_managed_session() {
    ...
    // when
    let output = run_crudo(&project_dir, &["--resume", "latest", "/status"]);
...
fn resume_latest_missing_session_fails_without_creating_session_dirs_435() {
    ...
    let text = run_crudo_with_env(&project_dir, &["--resume", "latest"], &envs);
    let json = run_crudo_with_env(
        &project_dir,
    ...
    assert!(
        !project_dir.join(".crudo").exists(),
        "failed resume must not create .crudo/session directories"
    );
}
...
fn resumed_status_command_emits_structured_json_when_requested() {
    ...
    // when
    // Use an isolated CRUDO_CONFIG_HOME so ~/.crudo/settings.json is not loaded,
...
fn run_crudo(current_dir: &Path, args: &[&str]) -> Output {
    run_crudo_with_env(current_dir, args, &[])
}
...
fn run_crudo_with_env(current_dir: &Path, args: &[&str], envs: &[(&str, &str)]) -> Output {

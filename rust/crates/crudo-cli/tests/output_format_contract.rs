...
fn export_help_preserves_plaintext_in_text_mode_384() {
    ...
    let output = run_crudo(&root, &["export", "--help"], &[]);
    assert!(
    ...
    assert!(stdout.contains("Usage            crudo export"));
...
fn assert_doctor_help_json_contract(parsed: &Value) {
    ...
    assert_eq!(parsed["usage"], "crudo doctor [--output-format <format>]");
...
fn doctor_help_text_stays_plaintext_and_local_702() {
    ...
    let output = run_crudo(&root, &["doctor", "--help"], &[]);
    assert!(
        output.status.success(),
        "stdout:\n{}\n\nstderr:\n{}",
        String::from_utf8_lossy(&output.stdout),
    ...
    assert!(stdout.contains("Usage            crudo doctor"));
...
fn resume_session_compact_help_short_circuits_before_config_or_auth_427() {
    ...
    fs::create_dir_all(root.join(".crudo")).expect("project config dir should exist");
    fs::create_dir_all(&config_home).expect("config home should exist");
    fs::create_dir_all(&home).expect("home should exist");
    fs::write(root.join(".crudo").join("settings.json"), "{").expect("broken config should write");
    ...
    for (args, heading) in text_cases {
        let output = run_crudo(&root, args, &envs);
        assert!(
            output.status.success(),
...
fn resume_missing_session_json_reports_local_store_before_auth_427() {
    ...
    assert!(
        parsed["sessions_dir"]
            .as_str()
            .is_some_and(|path| path.contains(".crudo") && path.contains("sessions")),
...
fn version_emits_json_when_requested() {
    ...
    assert!(
        parsed["human_readable"]
            .as_str()
            .is_some_and(|text| text.contains("Crudo")),
...
fn version_status_doctor_include_binary_provenance_797() {
    ...
    let git_commands: &[&[&str]] = &[
        &["config", "user.email", "test@crudo.test"],
...
fn direct_resume_safe_slash_commands_route_to_local_json_actions_831() {
    ...
    for (command, expected_kind, expected_status) in [
        ...
        let output = run_crudo(&root, &["--output-format", "json", command], &[]);
        assert!(
            output.status.success(),
...
fn global_cwd_flag_reports_typed_invalid_paths_429() {
    ...
    let empty_output = run_crudo(&root, &["--cwd", "", "status", "--output-format=json"], &[]);
...
fn export_invalid_output_path_reports_typed_json_430() {
    ...
    let directory_output = run_crudo(
        &root,
        &[
            "--output-format=json",
            "export",
...
fn status_json_accepts_namespaced_model_env_and_surfaces_alias_426() {
    ...
    assert_eq!(parsed["model_env_var"], "CRUDO_MODEL");
}
...
fn status_json_warns_on_invalid_model_env_426() {
    ...
    let output = run_crudo(&root, &["--output-format", "json", "status"], &envs);
    assert!(
        output.status.success(),
...
fn acp_guidance_emits_json_when_requested() {
    ...
    let output = run_crudo(&root, &["--output-format", "json", "acp"], &[]);
    assert_eq!(
        output.status.code(),
        Some(2),
...
fn inventory_commands_emit_structured_json_when_requested() {
    ...
    // use run_crudo directly instead of assert_json_command_with_env which checks success.
    let agents_show_out = run_crudo(
        &root,
        &[
            "--output-format",
            "json",
...
fn agents_command_emits_structured_agent_entries_when_requested() {
    ...
    assert_eq!(parsed["agents"][0]["source"]["id"], "project_crudo");
    ...
    assert_eq!(parsed["agents"][2]["shadowed_by"]["id"], "project_crudo");
}
...
fn agents_and_skills_inventory_share_source_schema_702() {
    ...
    assert_eq!(legacy_skill["source"]["id"], "project_crudo");
...
#[test]
fn memory_files_load_claude_crudo_agents_and_surface_json_438() {
    ...
    fs::write(root.join("CRUDO.md"), "MARKER-FROM-CRUDO-MD\n").expect("write CRUDO.md");
    ...
    let sources = memory_files
        ...
        .collect::<Vec<_>>();
    assert_eq!(sources, vec!["claude_md", "crudo_md", "agents_md"]);
    assert!(memory_files
        .iter()
    ...
    assert!(message.contains("MARKER-FROM-CRUDO-MD"));
    assert!(message.contains("MARKER-FROM-AGENTS-MD"));
    assert_eq!(prompt["memory_file_count"], 3);
    assert_eq!(prompt["memory_files"][1]["source"], "crudo_md");
...
fn mcp_json_reports_required_optional_and_redacts_secret_values() {
    ...
    let config_home = root.join("config-home");
    let home = root.join("home");
    fs::create_dir_all(root.join(".crudo")).expect("workspace config should exist");
    fs::create_dir_all(&config_home).expect("config home should exist");
    fs::create_dir_all(&home).expect("home should exist");
    fs::write(
...
fn config_json_reports_deprecations_structurally_without_stderr_duplicate_815() {
    ...
    let output = run_crudo(&root, &["--output-format", "json", "config"], &envs);
    assert!(
        output.status.success(),
        "stdout:\n{}\n\nstderr:\n{}",
        String::from_utf8_lossy(&output.stdout),
    ...
    let text_output = run_crudo(&root, &["config"], &envs);
    assert!(
...
fn status_deduplicates_config_deprecation_warnings_per_invocation_425() {
    ...
    let output = run_crudo(&root, &["status"], &envs);
    assert!(
        output.status.success(),
        "stdout:\n{}\n\nstderr:\n{}",
        String::from_utf8_lossy(&output.stdout),
...
fn config_json_attributes_precedence_and_shadowed_keys_425() {
    ...
    let legacy = files
        ...
        .find(|file| {
            file["source"] == "project"
                && file["path"]
                    .as_str()
                    .is_some_and(|path| path.ends_with(".crudo.json"))
        })
        .expect("project .crudo.json entry");
    let settings = files
        .iter()
        .find(|file| {
            file["source"] == "project"
                && file["path"]
                    .as_str()
                    .is_some_and(|path| path.ends_with(".crudo/settings.json"))
        })
        .expect("project .crudo/settings.json entry");
    ...
    assert!(
        ...
        "unshadowed legacy keys should remain attributed to .crudo.json: {legacy}"
    );
}
...
fn config_section_json_tolerates_unknown_keys_as_warnings_425() {
    ...
    fs::create_dir_all(&config_home).expect("config home should exist");
    fs::create_dir_all(&home).expect("home should exist");
    fs::write(root.join(".crudo.json"), r#"{"model":"opus","alpha":"x"}"#)
...
fn config_json_reports_structured_unloaded_file_reasons_407() {
    ...
    fs::create_dir_all(&config_home).expect("config home should exist");
    fs::create_dir_all(&home).expect("home should exist");
    fs::write(root.join(".crudo.json"), "{not json").expect("legacy skip fixture should write");
    fs::write(
        root.join(".crudo").join("settings.json"),
        r#"{"model":"opus"}"#,
    )
...
fn config_json_list_reports_parse_errors_without_dropping_file_statuses_407() {
    ...
    fs::write(root.join(".crudo").join("settings.json"), "{not json")
...
fn local_text_surface_preserves_config_deprecation_stderr_816() {
    ...
    let output = run_crudo(&root, &["doctor"], &envs);
    assert!(
        output.status.success(),
        "stdout:\n{}\n\nstderr:\n{}",
        String::from_utf8_lossy(&output.stdout),
...
fn assert_json_command_with_env(current_dir: &Path, args: &[&str], envs: &[(&str, &str)]) -> Value {
    let output = run_crudo(current_dir, args, envs);
    assert!(
        output.status.success(),
        "stdout:\n{}\n\nstderr:\n{}",
        String::from_utf8_lossy(&output.stdout),
...
fn run_crudo(current_dir: &Path, args: &[&str], envs: &[(&str, &str)]) -> Output {
    let mut command = Command::new(env!("CARGO_BIN_EXE_crudo"));
    command.current_dir(current_dir).args(args);
    for key in ["CRUDO_OUTPUT_FORMAT", "CRUDO_LOG", "RUST_LOG"] {
    ...
    command.output().expect("crudo should launch")
}
...
fn unique_temp_dir(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "crudo-output-format-{label}-{}-{millis}-{counter}",
        std::process::id()
    ))
}
...
fn diff_json_has_status_and_result_field_702() {
    // #458/#702: `crudo diff --output-format json` must have status ∈ {ok,error}
...
fn diff_json_changed_file_count_deduplication_733() {
    ...
    Command::new("git")
        ...
        .expect("git init");
    Command::new("git")
        .args(["config", "user.email", "test@crudo.test"])
        .current_dir(&root)
        .output()
    ...
    let bin = env!("CARGO_BIN_EXE_crudo");
    let clean = Command::new(bin)
        .current_dir(&root)
        .args(["--output-format", "json", "diff"])
        .output()
        .expect("crudo diff clean");
...
fn prompt_no_arg_json_error_kind_750() {
    // #751/#750/#823: `crudo prompt --output-format json` with no prompt argument must emit
    ...
    let output = Command::new(bin)
        ...
        .output()
        .expect("crudo prompt should run");
    assert!(
        !output.status.success(),
        "crudo prompt with no arg must exit non-zero"
    );
    assert_eq!(
        output.status.code(),
        Some(1),
        "crudo prompt with no arg must exit rc=1 (#823)"
    );
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert_eq!(
        stderr, "",
        "crudo prompt (no arg) --output-format json must keep stderr empty (#823); got: {stderr}"
    );
    let stdout = String::from_utf8_lossy(&output.stdout);
    let parsed: serde_json::Value = serde_json::from_str(stdout.trim()).unwrap_or_else(|_| {
        panic!(
            "crudo prompt (no arg) --output-format json must emit valid stdout JSON; got: {stdout}"
        )
    });
    assert_eq!(
        parsed["error_kind"], "missing_prompt",
        "crudo prompt no-arg must have error_kind:missing_prompt (#750/#823); got: {parsed}"
    );
    let hint = parsed["hint"].as_str().unwrap_or("");
    assert!(
        !hint.is_empty(),
        "crudo prompt no-arg hint must be non-empty (#750/#823)"
    );
    assert!(
        hint.contains("crudo prompt") || hint.contains("echo"),
        "hint should mention 'crudo prompt' or 'echo': {hint}"
    );
}

#[test]
fn prompt_empty_arg_json_stdout_missing_prompt_823() {
    // #823: `crudo --output-format json prompt ""` must match the missing prompt
    ...
    let output = Command::new(bin)
        ...
        .args(["--output-format", "json", "prompt", ""])
        .output()
        .expect("crudo prompt empty arg should run");
    assert_eq!(
        output.status.code(),
        Some(1),
        "crudo prompt empty arg must exit rc=1 (#823)"
    );
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert_eq!(
        stderr, "",
        "crudo prompt empty arg --output-format json must keep stderr empty (#823); got: {stderr}"
    );
    let stdout = String::from_utf8_lossy(&output.stdout);
    let parsed: serde_json::Value = serde_json::from_str(stdout.trim()).unwrap_or_else(|_| {
        panic!(
            "crudo prompt empty arg --output-format json must emit valid stdout JSON; got: {stdout}"
        )
    });
    assert_eq!(
        parsed["error_kind"], "missing_prompt",
        "crudo prompt empty arg must have error_kind:missing_prompt (#823); got: {parsed}"
    );
    assert_eq!(
        parsed["action"], "abort",
        "crudo prompt empty arg must retain abort action (#823); got: {parsed}"
    );
    assert!(
        parsed["hint"].as_str().map_or(false, |h| !h.is_empty()),
        "crudo prompt empty arg missing_prompt hint must be non-empty (#823)"
    );
}
...
fn flag_value_errors_have_error_kind_and_hint_756() {
    ...
    let out = Command::new(bin)
        ...
        .output()
        .expect("crudo --reasoning-effort HIGH should run");
    assert!(
        !out.status.success(),
    ...
    let out2 = Command::new(bin)
        ...
        .output()
        .expect("crudo --model (no value) should run");
    assert!(
        !out2.status.success(),
...
fn output_format_flags_and_env_have_typed_contract_433() {
    ...
    let invalid = run_crudo(&root, &["--output-format", "YAML", "status"], &[]);
    ...
    assert!(
        help_text.contains("CRUDO_OUTPUT_FORMAT"),
        "help should document CRUDO_OUTPUT_FORMAT: {help_text}"
    );
    assert!(
        help_text.contains("CRUDO_LOG"),
        "help should document CRUDO_LOG: {help_text}"
    );
    assert!(
    ...
    let doctor = assert_json_command_with_env(
        ...
        &["doctor"],
        &[
            ("CRUDO_OUTPUT_FORMAT", "json"),
            ("CRUDO_LOG", "debug"),
            ("RUST_LOG", "crudo=debug"),
        ],
    );
    let system_check = doctor["checks"]
        ...
        .find(|check| check["name"] == "system")
        .expect("system check");
    assert_eq!(system_check["crudo_output_format"], "json");
    assert_eq!(system_check["crudo_log"], "debug");
    assert_eq!(system_check["rust_log"], "crudo=debug");
}
...
fn short_p_flag_swallows_no_flags_755() {
    // #755: `crudo -p hello --output-format json` must parse --output-format json
    ...
    let output = Command::new(bin)
        ...
        .output()
        .expect("crudo -p should run");
    assert!(
        !output.status.success(),
        "crudo -p hello --output-format json must exit non-zero (no credentials)"
    );
    // #819/#820/#823: abort envelopes route to stdout in JSON mode
    ...
    let output2 = Command::new(bin)
        ...
        .output()
        .expect("crudo -p flag-as-prompt should run");
    let raw2 = String::from_utf8_lossy(&output2.stdout)
        ...
        .join("");
    let parsed2: serde_json::Value = serde_json::from_str(&raw2)
        .unwrap_or_else(|_| panic!("crudo -p --model must emit JSON to stdout; got: {raw2}"));
    assert_eq!(
...
fn short_p_flag_no_arg_json_error_kind_753() {
    // #753: `crudo --output-format json -p` (no prompt) must emit error_kind:"missing_prompt"
    ...
    assert!(
        !output.status.success(),
        "crudo -p with no arg must exit non-zero"
    );
    let stdout = String::from_utf8_lossy(&output.stdout);
    ...
    let parsed: serde_json::Value = serde_json::from_str(&raw).unwrap_or_else(|_| {
        panic!("crudo -p (no arg) --output-format json must emit valid JSON; got: {raw}")
    });
    assert_eq!(
        parsed["error_kind"], "missing_prompt",
        "crudo -p no-arg must have error_kind:missing_prompt (#753); got: {parsed}"
    );
    let hint = parsed["hint"].as_str().unwrap_or("");
    assert!(
        !hint.is_empty(),
        "crudo -p no-arg hint must be non-empty (#753)"
    );
    assert!(
        hint.contains("crudo -p") || hint.contains("crudo prompt"),
        "hint should mention 'crudo -p' or 'crudo prompt': {hint}"
    );
}

#[test]
fn bare_slash_command_hint_745() {
    // #747/#745: crudo <slash-cmd> --output-format json must return non-null hint.
    ...
    for cmd in &["issue", "pr", "commit"] {
        let output = Command::new(bin)
            ...
            .output()
            .expect("crudo should run");
        assert!(
            !output.status.success(),
            "crudo {cmd} outside REPL must exit non-zero"
        );
        ...
        let parsed: serde_json::Value = serde_json::from_str(&raw)
            .unwrap_or_else(|_| panic!("crudo {cmd} must emit JSON; got: {raw}"));
        assert_eq!(
            parsed["error_kind"], "interactive_only",
            "crudo {cmd} must have error_kind:interactive_only (#745)"
        );
        let hint = parsed["hint"].as_str().unwrap_or("");
        assert!(
            !hint.is_empty(),
            "crudo {cmd} --output-format json hint must be non-empty (#745); got null"
        );
    }
}
...
fn config_unsupported_section_json_hint_741() {
    // #744/#741: crudo config <unknown-section> --output-format json must return
...
fn config_help_returns_structured_section_list_344() {
    ...
    let output = Command::new(bin)
        ...
        .output()
        .expect("crudo config help should run");
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn export_json_has_kind_702() {
    // #458/#702: `crudo export --output-format json` must emit kind:export.
    ...
    let output = Command::new(bin)
        ...
        .env("ANTHROPIC_API_KEY", "test")
        .output()
        .expect("crudo binary should run");
...
fn config_parse_error_has_typed_error_kind_and_hint_764() {
    // #764: Malformed .crudo/settings.json must emit error_kind:config_parse_error
...
fn login_logout_removed_subcommands_have_error_kind_and_hint_765() {
    // #765: `crudo login` and `crudo logout` are removed; JSON envelope must carry
    ...
    for subcmd in &["login", "logout"] {
        let output = run_crudo(&root, &["--output-format", "json", subcmd], &[]);
        assert!(
            !output.status.success(),
            "crudo {subcmd} should exit non-zero"
        );
        let stdout = String::from_utf8_lossy(&output.stdout);
        let stderr = String::from_utf8_lossy(&output.stderr);
        let stdout = String::from_utf8_lossy(&output.stdout);
        let json_line = stdout
            .lines()
            .find(|l| l.trim_start().starts_with('{'))
            .unwrap_or_else(|| panic!("crudo {subcmd} stdout should contain a JSON envelope (#819/#820/#823: abort envelopes route to stdout in JSON mode)"));
        let parsed: serde_json::Value =
            serde_json::from_str(json_line).expect("error envelope should be valid JSON");

        assert_eq!(
            parsed["error_kind"], "removed_subcommand",
            "crudo {subcmd} must return error_kind:removed_subcommand (#765)"
        );
        let hint = parsed["hint"].as_str().unwrap_or("");
        assert!(
            !hint.is_empty(),
            "crudo {subcmd} must return non-null hint (#765), got: {hint:?}"
        );
        assert!(
            hint.contains("ANTHROPIC_API_KEY") || hint.contains("ANTHROPIC_AUTH_TOKEN"),
            "crudo {subcmd} hint must mention the env var migration path, got: {hint:?}"
        );
    }
}
...
fn diff_extra_args_have_typed_error_kind_and_hint_766() {
    // #766: `crudo diff --bogus` returned error_kind:"unknown" + hint:null.
...
fn diff_trailing_json_after_malformed_args_is_bounded_json_3129() {
    ...
    for (args, label) in [
        (
            ...
            "crudo diff --bogus-flag --output-format json",
        ),
        (
            &["diff", "does-not-exist", "--output-format", "json"][..],
            "crudo diff does-not-exist --output-format json",
        ),
        (
            &[
                "diff",
            ...
            "crudo diff --cached --bogus-flag --output-format json",
        ),
    ] {
...
fn assert_diff_unexpected_extra_args_json(root: &Path, args: &[&str], label: &str) {
    let output = run_crudo(root, args, &[]);
    assert!(
        !output.status.success(),
...
fn resume_non_slash_trailing_arg_has_typed_error_kind_and_hint_768() {
    // #768: `crudo --resume latest compact` (missing leading /) returned
    ...
    assert!(
        !output.status.success(),
        "crudo --resume latest compact should exit non-zero"
    );
    let stdout = String::from_utf8_lossy(&output.stdout);
    let stderr = String::from_utf8_lossy(&output.stderr);
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn session_with_unknown_subcommand_returns_interactive_only_not_credentials_767() {
    // #767: `crudo session bogus` bypassed all guards and fell through to
    ...
    for sub in &["bogus", "nuke", "delete-all"] {
        let output = run_crudo(&root, &["--output-format", "json", "session", sub], &[]);
        assert!(
            !output.status.success(),
            "crudo session {sub} should exit non-zero"
        );
        let stdout = String::from_utf8_lossy(&output.stdout);
        let stderr = String::from_utf8_lossy(&output.stderr);
        let stdout = String::from_utf8_lossy(&output.stdout);
        let json_line = stdout
            .lines()
            .find(|l| l.trim_start().starts_with('{'))
            .unwrap_or_else(|| panic!("crudo session {sub} stderr should contain JSON"));
        let parsed: serde_json::Value =
            serde_json::from_str(json_line).expect("error envelope should be valid JSON");

        assert_eq!(
            parsed["error_kind"], "interactive_only",
            "crudo session {sub} must return error_kind:interactive_only (#767), not missing_credentials"
        );
        let hint = parsed["hint"].as_str().unwrap_or("");
        assert!(
            !hint.is_empty(),
            "crudo session {sub} must return non-null hint (#767)"
        );
        assert!(
...
fn slash_only_verbs_with_args_return_interactive_only_not_credentials_770() {
    // #770: `crudo cost breakdown`, `crudo clear --force`, `crudo memory reset`,
    // and `crudo ultraplan bogus` all fell through to CliAction::Prompt and
    ...
    for args in cases {
        let full_args: Vec<&str> = std::iter::once("--output-format")
            ...
            .collect();
        let output = run_crudo(&root, &full_args, &[]);
        assert!(
            !output.status.success(),
            "crudo {} should exit non-zero",
            args.join(" ")
        );
        let stdout = String::from_utf8_lossy(&output.stdout);
        let stderr = String::from_utf8_lossy(&output.stderr);
        ...
        let json_line = stdout
            ...
            .unwrap_or_else(|| {
                panic!(
                    "crudo {} stderr should contain JSON, got: {stderr}",
                    args.join(" ")
                )
            });
        let parsed: serde_json::Value =
        ...
        assert_eq!(
            parsed["error_kind"],
            "interactive_only",
            "crudo {} must return error_kind:interactive_only (#770), not missing_credentials",
            args.join(" ")
        );
        let hint = parsed["hint"].as_str().unwrap_or("");
        assert!(
            !hint.is_empty(),
            "crudo {} must return non-null hint (#770)",
            args.join(" ")
        );
    }
}
...
fn agents_plugins_mcp_unknown_subcommand_have_hint_774() {
    // #774: `crudo agents bogus`, `crudo plugins bogus`, `crudo mcp bogus` returned
    ...
    // agents bogus
    {
        let output = run_crudo(&root, &["--output-format", "json", "agents", "bogus"], &[]);
    ...
    // plugins bogus
    {
        let output = run_crudo(&root, &["--output-format", "json", "plugins", "bogus"], &[]);
    ...
    // mcp bogus
    {
        let output = run_crudo(&root, &["--output-format", "json", "mcp", "bogus"], &[]);
...
fn mcp_show_missing_server_name_returns_missing_argument_830() {
    ...
    let output = run_crudo(&root, &["--output-format", "json", "mcp", "show"], &[]);
    assert!(
        !output.status.success(),
...
fn interactive_only_guard_batch_769_to_771() {
    ...
    assert!(
        ...
        "crudo model opus extra should exit non-zero"
    );
    ...
    let model_json: serde_json::Value = serde_json::from_str(model_stdout.trim())
        .unwrap_or_else(|_| panic!("crudo model opus extra should emit JSON, got: {model_stdout}"));
    assert_eq!(
        model_json["error_kind"], "unexpected_extra_args",
        "crudo model opus extra should now stay local and typed (#807), not missing_credentials: {model_json}"
    );
    assert!(
        model_json["hint"]
            .as_str()
            .is_some_and(|hint| !hint.is_empty()),
        "crudo model opus extra should include a usage hint: {model_json}"
    );
    ...
    for args in cases {
        ...
        let json_line = stdout
            ...
            .unwrap_or_else(|| {
                panic!(
                    "crudo {} should emit JSON, got stderr: {stderr}",
                    args.join(" ")
                )
            });
        let parsed: serde_json::Value = serde_json::from_str(json_line).unwrap();
        assert_eq!(
            parsed["error_kind"],
            "interactive_only",
            "crudo {} must return interactive_only, got {:?}",
            args.join(" "),
            parsed["error_kind"]
        );
        let hint = parsed["hint"].as_str().unwrap_or("");
...
fn resume_plugin_mutations_are_typed_interactive_only_777() {
    ...
    for mutation in &["install", "enable", "disable", "uninstall", "update"] {
        ...
        assert!(
            hint.contains("crudo") || hint.contains("REPL") || hint.contains("plugins"),
...
fn resume_skills_invocation_is_typed_interactive_only_779() {
    ...
    assert!(
        hint.contains("crudo") || hint.contains("REPL") || hint.contains("skills"),
...
fn acp_unsupported_invocation_has_hint_782() {
    // #782: `crudo acp start` returned error_kind:unsupported_acp_invocation but hint:null
    ...
    let output = run_crudo(&root, &["--output-format", "json", "acp", "start"], &[]);
...
fn init_json_envelope_has_hint_and_already_initialized_783() {
    // #783: crudo --output-format json init was missing the hint field entirely.
    ...
    let output = run_crudo(&root, &["--output-format", "json", "init"], &[]);
    ...
    assert_eq!(
        ...
        "fresh init should materialize .crudo/settings.json and safe .crudo.json"
    );
    assert_eq!(
        parsed["deferred"],
        json!([".crudo/sessions/"]),
    ...
    let crudo_json = fs::read_to_string(root.join(".crudo.json")).expect("read .crudo.json");
    assert!(
        crudo_json.contains("\"defaultMode\": \"acceptEdits\""),
        "init must not scaffold dontAsk in .crudo.json: {crudo_json}"
    );
    assert!(
        !crudo_json.contains("dontAsk"),
        "init must not scaffold unsafe dontAsk permission mode: {crudo_json}"
    );
    let settings_json = root.join(".crudo").join("settings.json");
    assert!(
        settings_json.is_file(),
        "init should template .crudo/settings.json"
    );
    assert!(
        !root.join(".crudo").join("sessions").exists(),
        "sessions directory should remain deferred until first save"
    );

    // Idempotent re-init — already_initialized should be true
    let output2 = run_crudo(&root, &["--output-format", "json", "init"], &[]);
    ...
    let existing_crudo_root = unique_temp_dir("init-existing-crudo-436");
    fs::create_dir_all(existing_crudo_root.join(".crudo")).expect("existing .crudo dir");
    let partial_output = run_crudo(
        &existing_crudo_root,
    ...
    assert!(
        ...
        "init with existing .crudo should succeed"
    );
    ...
    assert_eq!(
        partial["partial"],
        json!([".crudo/"]),
        "existing .crudo with newly-created settings should report partial .crudo/"
    );
    assert_eq!(
        partial["created"],
        json!([
        ...
        "init should still create missing sub-files when .crudo already exists"
    );
    assert!(
        existing_crudo_root
            .join(".crudo")
            .join("settings.json")
            .is_file(),
        "existing .crudo must receive missing settings template"
    );
}

#[test]
fn export_arg_errors_have_typed_kind_and_hint_784() {
    // #784: `crudo export --output` (missing flag value) returned error_kind:"unknown" + hint:null.
    // `crudo export a.md b.md` (extra positional) also returned unknown+null.
...
fn unknown_subcommand_returns_typed_kind_785() {
    // #785: `crudo dump` (a near-miss for dump-manifests) returned error_kind:"unknown"
    ...
    let output = run_crudo(&root, &["--output-format", "json", "dump"], &[]);
    ...
    assert!(
        hint.contains("dump-manifests") || hint.contains("--help") || hint.contains("crudo"),
...
fn dump_manifests_missing_dir_has_typed_kind_and_hint_786() {
    // #786: `crudo dump-manifests --manifests-dir` (no value) and `--manifests-dir=` (empty)
...
fn resume_directory_path_returns_typed_kind_and_hint_787() {
    // #787: `crudo --resume /tmp` (directory instead of .jsonl file) returned
...
fn skills_show_not_found_emits_single_json_object_788() {
    // #788: `crudo --output-format json skills show no-such-skill` emitted TWO JSON objects:
...
fn agents_show_not_found_exits_nonzero_789() {
    // #789: `crudo --output-format json agents show <not-found>` returned exit 0 despite
...
fn plugins_show_not_found_exits_nonzero_789() {
    // #789: same as agents — `crudo --output-format json plugins show <not-found>` exited 0
...
fn system_prompt_unknown_option_returns_typed_kind_790() {
    // #790: `crudo --output-format json system-prompt bogus` returned error_kind:"unknown" + hint:null.
    ...
    assert!(
        h1.contains("system-prompt") || h1.contains("crudo"),
...
fn config_extra_args_have_non_null_hint_791() {
    // #791: `crudo config show bogus-key` and `crudo config set a b` returned
    // error_kind:"unexpected_extra_args" + hint:null because the error message
    // "unexpected extra arguments after `crudo config ...`: ..." had no \n delimiter.
    ...
    assert!(
        h1.contains("config") || h1.contains("crudo"),
...
fn agents_list_flag_shaped_filter_returns_unknown_option_792() {
    // #792: `crudo --output-format json agents list --bogus-flag` silently returned
    ...
    assert!(
        h.contains("crudo agents list") || h.contains("filter"),
...
fn skills_list_flag_shaped_filter_returns_unknown_option_792() {
    // #792: same gap as agents — `crudo skills list --bogus-flag` returned success
    ...
    assert!(
        j["hint"]
            .as_str()
            .is_some_and(|h| h.contains("crudo skills list") || h.contains("filter")),
...
fn plugins_list_flag_shaped_filter_returns_cli_parse_on_stdout_793_817() {
    // #793: `crudo plugins list --bogus-flag` silently returned status:"ok" with empty
    ...
    assert!(
        h.contains("plugins list") || h.contains("filter") || h.contains("crudo"),
...
fn plugins_uninstall_not_found_has_hint_793() {
    // #793: `crudo plugins uninstall no-such-plugin` returned plugin_not_found + hint:null.
    ...
    assert!(
        h.contains("plugins list") || h.contains("crudo plugins"),
...
fn plugins_install_not_found_path_returns_typed_kind_794() {
    // #794: `crudo plugins install /nonexistent/path` returned error_kind:"unknown" + hint:null.
...
fn skills_lifecycle_errors_have_typed_local_json_795_431() {
    ...
    let missing_uninstall = run_crudo(
        &root,
        &[
            "skills",
            "uninstall",
...
fn agents_create_scaffolds_toml_and_lists_locally_431() {
    ...
    let agent_path = root.join(".crudo").join("agents").join("my-agent.toml");
...
fn agents_show_extra_positional_arg_returns_unexpected_extra_796() {
    // #796: `crudo agents show <name> <extra>` treated the full "name extra" as a single
    ...
    assert!(
        h.contains("crudo agents show") || h.contains("Usage"),
...
fn skills_show_extra_positional_arg_returns_unexpected_extra_796() {
    // #796: same gap as agents — `crudo skills show <name> <extra>` treated "name extra"
    ...
    assert!(
        j["hint"]
            .as_str()
            .is_some_and(|h| h.contains("crudo skills show") || h.contains("Usage")),
...
fn plugins_list_trailing_dash_text_error_stays_on_stderr_817() {
    ...
    let output = run_crudo(&root, &["plugins", "list", "--"], &[]);
    assert!(
        !output.status.success(),
    ...
    assert!(
        stderr.contains("unknown option for `crudo plugins list`: --"),
        "{stderr}"
    );
}
...
fn empty_prompt_has_non_null_hint_798() {
    // #798: `crudo --output-format json ""` returned empty_prompt + hint:null.
    ...
    let output = run_crudo(&root, &["--output-format", "json", ""], &[]);
    assert!(
        !output.status.success(),
    ...
    assert!(
        h.contains("crudo") || h.contains("Usage"),
        "hint should reference usage, got: {h:?}"
    );
}
...
fn diff_non_git_dir_has_error_kind_and_hint_801() {
    // #801: `crudo --output-format json diff` in a non-git directory returned
    ...
    let output = run_crudo(&root, &["--output-format", "json", "diff"], &[]);
...
fn models_json_and_model_help_json_are_local_807() {
    ...
    let models = run_crudo(&root, &["models", "--output-format", "json"], &[]);
    ...
    let help = run_crudo(&root, &["model", "help", "--output-format", "json"], &[]);
...
fn settings_json_and_help_json_are_local_808() {
    ...
    let settings = run_crudo(&root, &["settings", "--output-format", "json"], &[]);
    ...
    let help = run_crudo(&root, &["settings", "help", "--output-format", "json"], &[]);
    let help_json = assert_local_json_without_missing_credentials(&help, "help");
    assert_eq!(
...
fn unknown_subcommand_json_emits_command_not_found() {
    ...
    std::fs::create_dir_all(&root).expect("create temp dir");
    let output = run_crudo(&root, &["--output-format", "json", "foobar"], &[]);
    assert_eq!(
        output.status.code(),
        Some(1),
...
fn unknown_subcommand_text_emits_command_not_found_on_stderr() {
    ...
    std::fs::create_dir_all(&root).expect("create temp dir");
    let output = run_crudo(&root, &["foobar"], &[]);
    assert_eq!(
        output.status.code(),
        Some(1),
        "unknown subcommand should exit 1"
...
fn unknown_subcommand_typo_with_suggestions_json_emits_command_not_found() {
    ...
    std::fs::create_dir_all(&root).expect("create temp dir");
    let output = run_crudo(&root, &["--output-format", "json", "statuz"], &[]);
    assert_eq!(output.status.code(), Some(1));
    let stdout = String::from_utf8_lossy(&output.stdout);
    let stderr = String::from_utf8_lossy(&output.stderr);
    let j: serde_json::Value =
...
fn multi_word_unknown_subcommand_json_emits_command_not_found_826() {
    ...
    std::fs::create_dir_all(&root).expect("create temp dir");
    let output = run_crudo(&root, &["--output-format", "json", "foobar", "baz"], &[]);
    assert_eq!(output.status.code(), Some(1));
    let stdout = String::from_utf8_lossy(&output.stdout);
    let stderr = String::from_utf8_lossy(&output.stderr);
    let j: serde_json::Value =
    ...
    assert!(
        hint.contains("crudo prompt") || hint.contains("--help"),
...
fn compact_flag_missing_argument_and_shorthand_prompt_contract_435() {
    ...
    let missing = run_crudo(&root, &["--output-format", "json", "--compact"], &envs);
    assert_eq!(missing.status.code(), Some(1));
    assert!(
        missing.stderr.is_empty(),
...
fn direct_unknown_slash_command_emits_typed_error_kind() {
    ...
    std::fs::create_dir_all(&root).expect("create temp dir");
    let output = run_crudo(&root, &["--output-format", "json", "/boguscommand"], &[]);
...
fn approve_deny_outside_repl_emits_interactive_only() {
    ...
    for cmd in &["/approve", "/yes", "/deny", "/no"] {
        let output = run_crudo(&root, &["--output-format", "json", cmd], &[]);
        let stdout = String::from_utf8_lossy(&output.stdout);
        let stderr = String::from_utf8_lossy(&output.stderr);
        let j: serde_json::Value = serde_json::from_str(stdout.trim())
...
fn resume_safe_interactive_only_hint_includes_resume_suggestion() {
    ...
    std::fs::create_dir_all(&root).expect("create temp dir");
    let output = run_crudo(&root, &["--output-format", "json", "/compact"], &[]);
    let stdout = String::from_utf8_lossy(&output.stdout);
    let j: serde_json::Value = serde_json::from_str(stdout.trim())

...
fn compact_slash_command_in_repl_does_not_start_nested_tokio_runtime() {
    ...
    // when
    let output = run_crudo_repl(&workspace, &config_home, &home, "/compact\n/exit\n");
...
fn run_crudo_repl(
    ...
    let mut command = python_pty_command(env!("CARGO_BIN_EXE_crudo"));
    ...
    child.wait_with_output().expect("crudo should finish")
}

fn python_pty_command(crudo: &str) -> Command {
...
child = subprocess.Popen([crudo], stdin=slave, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
os.close(slave)

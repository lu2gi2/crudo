...
fn status_command_applies_model_and_permission_mode_flags() {
    ...
    // when
    let output = Command::new(env!("CARGO_BIN_EXE_crudo"))
...
fn slash_command_names_match_known_commands_and_suggest_nearby_unknown_ones() {
    ...
    // when
    let help_output = Command::new(env!("CARGO_BIN_EXE_crudo"))
        .current_dir(&temp_dir)
        .arg("/help")
        .output()
        .expect("crudo should launch");
    let unknown_output = Command::new(env!("CARGO_BIN_EXE_crudo"))
        .current_dir(&temp_dir)
        .arg("/zstats")
        .output()
        .expect("crudo should launch");
...
fn config_command_loads_defaults_from_standard_config_locations() {
    ...
    let config_home = temp_dir.join("home").join(".crudo");
    fs::create_dir_all(temp_dir.join(".crudo")).expect("project config dir should exist");
    ...
    fs::write(temp_dir.join(".crudo.json"), r#"{"model":"sonnet"}"#)
        .expect("write project settings");
    fs::write(
        temp_dir.join(".crudo").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
    ...
    let output = command_in(&temp_dir)
        .env("CRUDO_CONFIG_HOME", &config_home)
        .args([
            "--resume",
            session_path.to_str().expect("utf8 path"),
            "/config",
    ...
    assert!(stdout.contains(temp_dir.join(".crudo.json").to_str().expect("utf8 path")));
    assert!(stdout.contains(
        temp_dir
            .join(".crudo")
...
fn doctor_command_runs_as_a_local_shell_entrypoint() {
    ...
    let output = command_in(&temp_dir)
        ...
        .arg("doctor")
        .output()
        .expect("crudo doctor should launch");
...
fn local_smoke_commands_do_not_require_live_credentials() {
    ...
    let config_home = temp_dir.join("home with spaces").join(".crudo");
    fs::create_dir_all(&config_home).expect("config home should exist");
    fs::create_dir_all(&temp_dir).expect("temp dir should exist");
    ...
    for args in [
        ...
        let output = offline_command_in(&temp_dir, &config_home)
            .args(args)
            .output()
            .unwrap_or_else(|error| panic!("crudo {args:?} should launch: {error}"));

        assert_success(&output);
        let stdout = String::from_utf8(output.stdout).expect("stdout should be utf8");
        let stderr = String::from_utf8(output.stderr).expect("stderr should be utf8");
        assert!(
            stdout.contains("crudo")
...
fn local_subcommand_help_does_not_fall_through_to_runtime_or_provider_calls() {
    ...
    assert!(doctor_stdout.contains("Usage            crudo doctor"));
    ...
    assert!(status_stdout.contains("Usage            crudo status"));

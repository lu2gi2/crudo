...
fn compact_flag_prints_only_final_assistant_text_without_tool_call_details() {
    ...
    // when we run crudo in compact text mode against a tool-using scenario
...
fn compact_flag_streaming_text_only_emits_final_message_text() {
    ...
    // when we invoke crudo with --compact for the streaming text scenario
...
fn text_prompt_mode_prints_final_assistant_text_after_spinner() {
    ...
    // when we invoke crudo in normal text prompt mode for the streaming text scenario
    let prompt = format!("{SCENARIO_PREFIX}streaming_text");
    let output = run_crudo(
        &workspace,
        &config_home,
...
fn prompt_subcommand_reads_prompt_from_stdin_when_no_positional_arg_423() {
    ...
    let prompt = format!("{SCENARIO_PREFIX}streaming_text\n");
    let output = run_crudo_with_stdin(
        &workspace,
        &config_home,
        &home,
        &base_url,
...
fn compact_subcommand_json_fails_fast_when_stdin_closed() {
    ...
    let output = run_crudo_closed_stdin_with_timeout(
        &workspace,
        &config_home,
        &home,
...
fn compact_subcommand_text_fails_fast_when_stdin_closed() {
    ...
    assert!(stderr.contains("crudo compact"), "{stderr}");
...
fn run_crudo_with_stdin(
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_crudo"))
        .current_dir(cwd)
        .env_clear()
        .env("ANTHROPIC_API_KEY", "test-compact-key")
        .env("ANTHROPIC_BASE_URL", base_url)
...
fn run_crudo_closed_stdin_with_timeout(
    cwd: &std::path::Path,
    config_home: &std::path::Path,
    home: &std::path::Path,
    args: &[&str],
...
fn unique_temp_dir(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "crudo-compact-{label}-{}-{millis}-{counter}",
        std::process::id()
    ))
}

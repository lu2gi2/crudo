...
const STARTER_CRUDO_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
...
impl InitStatus {
    ...
    /// Unlike `label()`, this never changes wording: crudos can switch on
...
impl InitReport {
    ...
    /// Summary constant that crudos can embed in JSON output without having
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let crudo_dir = cwd.join(".crudo");
    let crudo_dir_status = ensure_dir(&crudo_dir)?;
    let settings_json = crudo_dir.join("settings.json");
    let settings_status = write_file_if_missing(&settings_json, STARTER_SETTINGS_JSON)?;
    let crudo_dir_status =
        if crudo_dir_status == InitStatus::Skipped && settings_status == InitStatus::Created {
            InitStatus::Partial
        } else {
            crudo_dir_status
        };
    artifacts.push(InitArtifact {
        name: ".crudo/",
        status: crudo_dir_status,
    });
    artifacts.push(InitArtifact {
        name: ".crudo/settings.json",
        status: settings_status,
    });
    artifacts.push(InitArtifact {
        name: ".crudo/sessions/",
        status: if crudo_dir.join("sessions").is_dir() {
            InitStatus::Skipped
        } else {
            InitStatus::Deferred
        },
    ...
    let crudo_json = cwd.join(".crudo.json");
    artifacts.push(InitArtifact {
        name: ".crudo.json",
        status: write_file_if_missing(&crudo_json, STARTER_CRUDO_JSON)?,
    });
...
pub(crate) fn render_init_claude_md(cwd: &Path) -> String {
    ...
    let mut lines = vec![
        "# CLAUDE.md".to_string(),
        String::new(),
        "This file provides guidance to Crudo (crudo.dev) when working with code in this repository.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.crudo.json`; reserve `.crudo/settings.local.json` for machine-local overrides.".to_string());
...
mod tests {
    ...
    fn temp_dir() -> std::path::PathBuf {
        ...
        std::env::temp_dir().join(format!("crudo-init-{nanos}-{id}"))
    }
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".crudo/"));
        assert!(rendered.contains(".crudo.json"));
        assert!(rendered.contains("created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("CLAUDE.md        created"));
        assert!(root.join(".crudo").is_dir());
        assert!(root.join(".crudo.json").is_file());
        assert!(root.join("CLAUDE.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".crudo.json")).expect("read crudo json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"acceptEdits\"\n",
        ...
        assert_eq!(
            fs::read_to_string(root.join(".crudo").join("settings.json"))
        ...
        assert!(gitignore.contains(".crudo/settings.local.json"));
        assert!(gitignore.contains(".crudo/sessions/"));
        assert!(gitignore.contains(".crudohip/"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        assert_eq!(
            ...
            vec![".crudo/".to_string()],
            "existing .crudo/ should report partial when init creates missing settings.json"
        );
        assert!(root.join(".crudo").join("settings.json").is_file());

        let second = initialize_repo(&root).expect("second init should succeed");
        let second_rendered = second.render();
        assert!(second_rendered.contains(".crudo/"));
        assert!(second_rendered.contains(".crudo/settings.json"));
        assert!(second_rendered.contains(".crudo/sessions/"));
        assert!(second_rendered.contains(".crudo.json"));
        ...
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".crudo/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".crudo/sessions/").count(), 1);
        assert_eq!(gitignore.matches(".crudohip/").count(), 1);
    ...
    fn artifacts_with_status_partitions_fresh_and_idempotent_runs() {
        ...
        assert_eq!(
            ...
            vec![
                ".crudo/".to_string(),
                ".crudo/settings.json".to_string(),
                ".crudo.json".to_string(),
                ".gitignore".to_string(),
                "CLAUDE.md".to_string(),
            ],

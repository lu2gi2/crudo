...
pub fn run_setup_wizard() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!();
    println!("  \x1b[1mCrudo Setup Wizard\x1b[0m");
    ...
    println!();
    println!("  \x1b[32mProvider saved to ~/.crudo/settings.json\x1b[0m");
    println!(
        "  Run \x1b[1m/model {}\x1b[0m or restart crudo to activate.",
...
fn load_current_settings_field(field: &str) -> Option<String> {
    ...
    let settings_path = std::path::Path::new(&home).join(".crudo/settings.json");
...
fn save_settings_field(field: &str, value: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let settings_dir = std::path::Path::new(&home).join(".crudo");

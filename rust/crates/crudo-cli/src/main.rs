...
/// #148: Model provenance for `crudo status` JSON/text output. Records where
/// the resolved model string came from so crudos don't have to re-read argv
...
enum ModelSource {
    ...
    Env,
    /// `model` key in `.crudo.json` / `.crudo/settings.json` (when neither
    /// flag nor env set it).
    Config,
...
fn env_model_for_runtime() -> Option<EnvModel> {
    ["CRUDO_MODEL", "ANTHROPIC_MODEL", "ANTHROPIC_DEFAULT_MODEL"]
        .into_iter()
        .find_map(|name| {
            env::var(name)
                .ok()
...
const OFFICIAL_REPO_URL: &str = "https://github.com/ultraworkers/crudo";
const OFFICIAL_REPO_SLUG: &str = "ultraworkers/crudo";
const DEPRECATED_INSTALL_COMMAND: &str = "cargo install crudo";
...
fn main() {
    if let Err(error) = run() {
        ...
        if json_output {
            // #77/#696: classify error by prefix so downstream crudos can route
            ...
            let kind = classify_error_kind(&message);
            if message.contains("`crudo --help`") {
                eprintln!(
                    "[error-kind: {kind}]
error: {message}"
                );
...
fn classify_error_kind(message: &str) -> &'static str {
    ...
    } else if message.contains(".crudo/settings.json") || message.contains(".crudo.json") {
        ...
        || message.starts_with("skill '")
    {
        ...
        // #785/#825: typo/unknown top-level subcommand (e.g. `crudo dump` → did you mean dump-manifests?)
    ...
    {
        // #766: extra positionals after commands that take no arguments (e.g. crudo diff)
...
fn fallback_hint_for_error_kind(kind: &str) -> Option<&'static str> {
    match kind {
        ...
        "missing_credentials" => {
            Some("Set ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN before running crudo.")
        }
        "config_parse_error" => Some(
            "Fix the JSON syntax or schema in the referenced .crudo/settings.json or .crudo.json file, then rerun the command.",
        ),
        // #787: session load failures have no \n-delimited hint from the OS error path
        "session_load_failed" => Some(
            "Pass a path to a .jsonl session file, not a directory. Managed sessions live in .crudo/sessions/.",
        ),
        "session_path_is_directory" => Some(
            "--resume expects a .jsonl session file path, not a directory. Run `crudo --output-format json /session list` to list managed sessions.",
        ),
        ...
        "plugin_not_found" => Some("Run `crudo plugins list` to see installed plugins."),
        ...
        "skill_not_found" => Some(
            "Run `crudo skills list` to see available skills, or `crudo skills install <path>` to install a new one.",
        ),
        ...
        "unsupported_skills_action" => Some(
            "Supported: list, show <name>, install <path>, uninstall <name>, help. Run `crudo skills help` for details.",
        ),
...
impl std::fmt::Display for InvalidOutputPathError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "invalid_output_path: {}: `{}`\nUsage: crudo export [PATH] [--session SESSION] [--output PATH]",
            self.reason.as_str(),
            self.path
        )
    }
...
enum CliAction {
    ...
    // #146: `crudo config` and `crudo diff` are pure-local read-only
...
enum LocalHelpTopic {
    ...
    // `crudo <subcommand> --help` has one consistent contract.
    Init,
    State,
    Resume,
    Session,
    ...
    DumpManifests,
    BootstrapPlan,
    // #720: subsystem help topics so `crudo help agents` etc. route to usage JSON
    Agents,
    Skills,
    Plugins,
    Mcp,
...
fn raw_args_request_json_output(args: &[String]) -> bool {
    ...
    env::var("CRUDO_OUTPUT_FORMAT").ok().is_some_and(|value| {
        let value = value.trim();
...
fn output_format_selection_from_env() -> Result<OutputFormatSelection, String> {
    match env::var("CRUDO_OUTPUT_FORMAT") {
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
                    ...
                    && matches!(rest[0].as_str(), "prompt" | "commit" | "pr" | "issue") =>
            {
                ...
                // the arg to the API (e.g. `crudo prompt --help`) should show
            ...
            "-p" => {
                // Crudo compat: -p "prompt" = one-shot prompt.
            ...
            "--print" => {
                // Crudo compat: --print makes output non-interactive
    ...
    if rest.is_empty() {
        ...
        if !stdin_is_terminal {
            ...
            return Err("interactive_only: crudo requires an interactive terminal.\nStdin is not a TTY and no prompt was provided — pipe a prompt with `echo 'task' | crudo` or run `crudo` in an interactive terminal.".into());
        }
    ...
    // #696: `crudo compact` is the bare name of the interactive `/compact`
    ...
    // startup. `crudo --resume SESSION.jsonl /compact` remains the supported
    ...
    // `crudo --output-format json config`) can report config warnings
    ...
    match rest[0].as_str() {
        ...
        // top-level parser arm produced CliAction::Plugins. That made `crudo
        // plugins` (and `crudo plugins --help`, `crudo plugins list`, ...)
        ...
        "plugins" | "plugin" | "marketplace" => {
            ...
            if tail.len() > 2 {
                ...
                return Err(format!(
                    "unexpected extra arguments after `crudo {} {}`: {}\nUsage: crudo plugins [list|show <id>|install <id>|enable <id>|disable <id>|uninstall <id>|update <id>|help]",
                    rest[0],
                    tail[..2].join(" "),
                    tail[2..].join(" ")
                ));
        ...
        // `.crudo.json` + `.crudo/settings.json` from disk, no network, no
        // state mutation). Previously callers had to spin up a session with
        // `crudo --resume SESSION.jsonl /config` to see their own config,
        ...
        "config" => {
            ...
            if tail.len() > 1 {
                ...
                return Err(format!(
                    "unexpected extra arguments after `crudo config {}`: {}\nUsage: crudo config [env|hooks|model|plugins|mcp|settings]",
                    tail[0],
                    tail[1..].join(" ")
                ));
            }
        ...
        // `crudo permissions <mode>` falls through to the LLM when called
        ...
        "permissions" => Err(
            "`crudo permissions` is a slash command. Start `crudo` and run `/permissions` inside the REPL.\n  Usage  /permissions [read-only|workspace-write|danger-full-access]"
                .to_string(),
        ),
        // #767: `crudo session bogus` bypassed parse_single_word_command_alias (rest.len()>1),
        ...
        "session" => {
            // #449: `crudo session list` is a pure local filesystem read that
            ...
            if rest.get(1).map(|s| s.as_str()) == Some("list") {
                ...
                Err(format!(
                    "interactive_only: `crudo session` is a slash command{action_hint}.\nUse `crudo --resume SESSION.jsonl /session <action>` or start `crudo` and run `/session [list|exists|switch|fork|delete]`."
                ))
            }
        }
        ...
        "cost" => Err(
            "interactive_only: `crudo cost` is a slash command.\nUse `crudo --resume SESSION.jsonl /cost` or start `crudo` and run `/cost`."
                .to_string(),
        ),
        "clear" => Err(
            "interactive_only: `crudo clear` is a slash command.\nUse `crudo --resume SESSION.jsonl /clear [--confirm]` or start `crudo` and run `/clear`."
                .to_string(),
        ),
        "memory" => Err(
            "interactive_only: `crudo memory` is a slash command.\nStart `crudo` and run `/memory` inside the REPL."
                .to_string(),
        ),
        "ultraplan" => Err(
            "interactive_only: `crudo ultraplan` is a slash command.\nStart `crudo` and run `/ultraplan` inside the REPL."
                .to_string(),
        ),
        "model" | "models" => {
            let tail = &rest[1..];
            ...
            if tail.len() > 1 {
                return Err(format!(
                    "unexpected extra arguments after `crudo {} {}`: {}\nUsage: crudo {} [help] [--output-format json]",
                    rest[0],
                    tail[0],
                    tail[1..].join(" "),
                    rest[0]
        ...
        "usage" => Err(
            "interactive_only: `crudo usage` is a slash command.\nUse `crudo --resume SESSION.jsonl /usage` or start `crudo` and run `/usage`."
                .to_string(),
        ),
        "stats" => Err(
            "interactive_only: `crudo stats` is a slash command.\nUse `crudo --resume SESSION.jsonl /stats` or start `crudo` and run `/stats`."
                .to_string(),
        ),
        "fork" => Err(
            "interactive_only: `crudo fork` is a slash command.\nStart `crudo` and run `/session fork [branch-name]` inside the REPL."
                .to_string(),
        ),
        "skills" => {
        ...
        "settings" => {
            ...
            if tail.is_empty() {
                ...
                Err(format!(
                    "unexpected extra arguments after `crudo settings`: {}\nUsage: crudo settings [help] [--output-format json]",
                    tail.join(" ")
                ))
            }
        }
        ...
        "init" => {
            ...
            if rest.len() > 1 {
                ...
                return Err(format!(
                    "unexpected extra arguments after `crudo init`: {extra}\nUsage: crudo init [--cwd <dir>] [--date <date>] [--session <session-id>]"
                ));
            }
        ...
        "setup" => {
            if rest.len() > 1 {
                let extra = rest[1..].join(" ");
                return Err(format!(
                    "unexpected extra arguments after `crudo setup`: {extra}\nUsage: crudo setup"
                ));
            }
...
Usage: crudo prompt <text>  or  echo '<text>' | crudo prompt".to_string());
            }
            Ok(CliAction::Prompt {
                prompt,
                model,
        ...
        other => {
                    ...
                    || (output_format == CliOutputFormat::Json && model_flag_raw.is_none()))
            {
                ...
                message.push_str(
                    "\nRun `crudo --help` for the full list. If you meant to send a prompt literally, use `crudo prompt <text>`.",
                );
                return Err(message);
            }
            // #147: guard empty/whitespace-only prompts at the fallthrough
            // path the same way `"prompt"` arm above does. Without this,
            // `crudo ""`, `crudo "   "`, and `crudo "" ""` silently route to
            ...
            if joined.trim().is_empty() {
                ...
                return Err(
                    "empty prompt: provide a subcommand or a non-empty prompt string.\nUsage: crudo <subcommand> or crudo -p <prompt>. Run `crudo --help` for the full list."
                        .to_string(),
                );
            }
            Ok(CliAction::Prompt {
...
fn parse_single_word_command_alias(
    ...
    if is_diagnostic && rest.len() > 1 {
        ...
        // #720: `crudo help <topic>` — when the verb is "help" and exactly one
        ...
        if rest[1] == "--json" {
            ...
            msg.push_str(&format!("\nRun `crudo {} --help` for usage.", verb));
        }
        return Some(Err(msg));
    }

    // #720: `crudo help <topic>` — when `help` is the verb and a topic follows,
    ...
    // #453: fire guard for multi-word CLI subcommands too (crudo cost list, crudo model list, etc.)
...
fn bare_slash_command_guidance(command_name: &str) -> Option<String> {
    ...
    let guidance = if slash_command.resume_supported {
        format!(
            "`crudo {command_name}` is a slash command.\nUse `crudo --resume SESSION.jsonl /{canonical_name}` or start `crudo` and run `/{canonical_name}`."
        )
    } else {
        format!(
            "`crudo {command_name}` is a slash command.\nStart `crudo` and run `/{canonical_name}` inside the REPL."
        )
    };
...
fn compact_interactive_only_error() -> String {
    ...
    "interactive_only: `crudo compact` is an interactive/session command.\nStart `crudo` and run `/compact`, or use `crudo --resume SESSION.jsonl /compact` to compact an existing session."
        .to_string()
}
    ...
    match SlashCommand::parse(&raw) {
        ...
        Ok(Some(SlashCommand::Unknown(name))) => {
            ...
            if matches!(name.as_str(), "approve" | "yes" | "y" | "deny" | "no" | "n") {
                Err(format!(
                    "interactive_only: /{name} requires an active tool call in the REPL.\nStart `crudo` and use /{name} to approve or deny a pending tool execution."
                ))
            } else {
...
fn format_unknown_option(option: &str) -> String {
    if option == "--" {
        return "end_of_flags: `--` terminates flag parsing. Pass literal prompt text after it, for example `crudo -- \"-literal prompt\"`.\nRun `crudo --help` for usage.".to_string();
    }
...
fn format_unknown_direct_slash_command(name: &str) -> String {
    ...
    message.push_str("\nRun `crudo --help` for CLI usage, or start `crudo` and use /help.");
    message
}
...
fn omc_compatibility_note_for_unknown_slash_command(name: &str) -> Option<&'static str> {
    name.starts_with("oh-my-claudecode:")
        .then_some(
            "Compatibility note: `/oh-my-claudecode:*` is a Claude Code/OMC plugin command. `crudo` does not yet load plugin slash commands, Claude statusline stdin, or OMC session hooks.",
        )
}
    ...
    while index < args.len() {
        match args[index].as_str() {
            "--cwd" => {
                ...
                if !cwd.exists() {
                    return Err(format!(
                        "invalid_cwd: path '{value}' does not exist.\nUsage: crudo system-prompt --cwd <existing-directory>"
                    ));
                }
                if !cwd.is_dir() {
                    return Err(format!(
            ...
            other => {
                ...
                let hint = if other == "--json" {
                    "Did you mean `--output-format json`? Usage: crudo system-prompt [--cwd <dir>] [--date <YYYY-MM-DD>] [--output-format text|json]".to_string()
                } else {
                    "Usage: crudo system-prompt [--cwd <dir>] [--date <YYYY-MM-DD>] [--output-format text|json]".to_string()
                };
                return Err(format!(
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--output" | "-o" => {
                let value = args
                    .get(index + 1)
                    .ok_or_else(|| format!("missing_flag_value: missing value for {}.\nUsage: crudo export [PATH] [--session SESSION] [--output PATH]", args[index]))?;
...
fn parse_resume_args(
    ...
    for token in command_tokens {
        ...
        if current_command.is_empty() {
            ...
            return Err(format!(
                "invalid_resume_argument: `{token}` is not a slash command.\nUsage: crudo --resume <session-id|latest> /<slash-command>  (e.g. /compact, /status)"
            ));
        }
...
/// Starts a minimal Model Context Protocol server that exposes crudo's
...
/// Read `.crudo/worker-state.json` from the current working directory and print it.
/// This is the file-based worker observability surface: `push_event()` in `worker_boot.rs`
/// atomically writes state transitions here so external observers (crudohip, orchestrators)
...
fn run_worker_state(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let state_path = cwd.join(".crudo").join("worker-state.json");
    if !state_path.exists() {
        ...
        // no `crudo worker` subcommand), so crudos/CI had no discoverable path
        ...
        //     Run:   crudo               # start the REPL (writes state on first turn)
        //     Or:    crudo prompt <text> # run one non-interactive turn
        //     Then rerun: crudo state [--output-format json]
        return Err(format!(
...
fn check_auth_health() -> DiagnosticCheck {
    ...
    match load_oauth_credentials() {
        ...
        .with_details(vec![
            ...
            "Suggested action  set ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN; `crudo login` is removed"
                .to_string(),
        ])
...
fn check_mcp_validation_health(summary: &McpValidationSummary) -> DiagnosticCheck {
    ...
    .with_hint(if summary.has_invalid_servers() {
        "Inspect `crudo mcp list --output-format json` invalid_servers and fix each rejected mcpServers entry."
    } else {
        ""
    })
    .with_details(details)
...
fn check_hook_validation_health(summary: &HookValidationSummary) -> DiagnosticCheck {
    ...
    .with_hint(if summary.has_invalid_hooks() {
        "Inspect `crudo status --output-format json` hook_validation.invalid_hooks and fix each rejected hooks entry."
    } else {
        ""
    })
    .with_details(details)
...
fn check_install_source_health() -> DiagnosticCheck {
    ...
    .with_details(vec![
        ...
        "Recommended path  build from this repo or use the upstream binary documented in README.md"
            .to_string(),
        format!(
            "Deprecated crate  `{DEPRECATED_INSTALL_COMMAND}` installs a deprecated stub and does not provide the `crudo` binary"
        )
            .to_string(),
    ])
    .with_data(Map::from_iter([
...
fn check_memory_health(context: &StatusContext) -> DiagnosticCheck {
    ...
    .with_hint(if has_outside_project {
        "Inspect workspace.memory_files in `crudo status --output-format json`; move unintended ancestor instructions inside the git project or run from the intended workspace root."
    } else if has_unloaded {
        "Move instructions into CLAUDE.md, CRUDO.md, or AGENTS.md within the current workspace ancestry, or inspect workspace.memory_files in `crudo status --output-format json`."
    } else {
        ""
    })
    .with_details(details)
...
fn check_system_health(cwd: &Path, config: Option<&runtime::RuntimeConfig>) -> DiagnosticCheck {
    ...
    let mut details = vec![
        ...
        format!(
            "Output format env  CRUDO_OUTPUT_FORMAT={}",
            env::var("CRUDO_OUTPUT_FORMAT").unwrap_or_else(|_| "<unset>".to_string())
        ),
        format!(
            "Logging env      CRUDO_LOG={} RUST_LOG={}",
            env::var("CRUDO_LOG").unwrap_or_else(|_| "<unset>".to_string()),
    ...
    .with_data(Map::from_iter([
        ...
        (
            "crudo_output_format".to_string(),
            json!(env::var("CRUDO_OUTPUT_FORMAT").ok()),
        ),
        ("crudo_log".to_string(), json!(env::var("CRUDO_LOG").ok())),
                ...
                if output_format == CliOutputFormat::Json {
                    println!(
                        ...
                        serde_json::json!({
                            ...
                            "hint": "This command is not available in the current build. Update crudo or use a different command.",
                            "exit_code": 2,
                            "command": raw_command,
                        })
                    );
...
fn memory_scope_path(path: &Path) -> PathBuf {
    ...
    if matches!(parent_name, Some(".crudo" | ".claude")) {
        return parent.parent().unwrap_or(parent).to_path_buf();
    }
    if matches!(parent_name, Some("rules" | "rules.local")) {
        if let Some(grandparent) = parent.parent() {
            if grandparent.file_name().and_then(|name| name.to_str()) == Some(".crudo") {
...
fn unloaded_memory_candidates(
    ...
    while let Some(dir) = cursor {
        for name in ["CRUDO.md", "AGENTS.md"] {
...
struct StatusContext {
    ...
    /// #143: when `.crudo.json` (or another loaded config file) fails to parse,
    ...
    /// `status: "degraded"` so crudos can distinguish "status ran but config
    ...
    /// readable string so downstream crudos can switch on the kind token
...
fn render_resume_usage() -> String {
  ...
  Auto-save        .crudo/sessions/<workspace-fingerprint>/<session-id>.{PRIMARY_SESSION_EXTENSION}
...
fn last_failed_boot_reason(cwd: &Path) -> Option<String> {
    env::var("CRUDO_LAST_FAILED_BOOT_REASON")
        .ok()
        .filter(|value| !value.trim().is_empty())
        .or_else(|| {
            fs::read_to_string(cwd.join(".crudo").join("last-failed-boot.txt"))
                .ok()
                .map(|value| value.trim().to_string())
                .filter(|value| !value.is_empty())
        })
    ...
    match command {
        ...
        SlashCommand::Skills { args } => {
            if let SkillSlashDispatch::Invoke(_) = classify_skills_slash_command(args.as_deref()) {
                ...
                return Err(format!(
                    "interactive_only: /skills {skill_name} invocation requires a live session.\nStart `crudo` and run `/skills {skill_name}` inside the REPL, or use `crudo -p <prompt>` with skill context."
                ).into());
            }
            let cwd = env::current_dir()?;
            Ok(ResumeCommandOutcome {
        ...
        SlashCommand::Plugins { action, target } => {
            ...
            match action.as_deref() {
                Some(action @ ("install" | "uninstall" | "enable" | "disable" | "update")) => {
                    ...
                    return Err(format!(
                        "interactive_only: /plugins {action} requires a live session to reload the plugin runtime.\nStart `crudo` and run `/plugins {action}` inside the REPL, or use `crudo plugins {action}` as a direct CLI command."
                    ).into());
                }
                _ => {}
            }
...
fn enforce_broad_cwd_policy(
    ...
    if is_interactive {
        ...
        eprintln!(
            "Warning: crudo is running from a very broad directory ({}).\n\
             The agent can read and search everything under this path.\n\
             Consider running from inside your project: cd /path/to/project && crudo",
            cwd.display()
        );
        ...
        let message = format!(
            "crudo is running from a very broad directory ({}). \
             The agent can read and search everything under this path. \
             Use --allow-broad-cwd to proceed anyway, \
             or run from inside your project: cd /path/to/project && crudo",
            cwd.display()
        );
        match output_format {
            CliOutputFormat::Json => {
 ...
 ╚═════╝╚═╝  ╚═╝ ╚═════╝ ╚═════╝  ╚═════╝\x1b[0m \x1b[38;5;183mCrudo\x1b[0m ◈\n\n\
  ...
  Type \x1b[1m/help\x1b[0m for commands · \x1b[1m/status\x1b[0m for live context · \x1b[2m/resume latest\x1b[0m jumps back to the newest session · \x1b[1m/diff\x1b[0m then \x1b[1m/commit\x1b[0m to ship · \x1b[2mTab\x1b[0m for workflow completions · \x1b[2mShift+Enter\x1b[0m for newline",
    ...
    fn print_mcp(
        ...
        // `crudo mcp serve` starts a stdio MCP server exposing crudo's built-in
                ...
                if filter.starts_with('-') {
                    if matches!(output_format, CliOutputFormat::Json) {
                        ...
                        let obj = json!({
                            ...
                            "status": "error",
                            "error_kind": "cli_parse",
                            "error": format!("unknown option for `crudo plugins list`: {filter}"),
                            "message": format!("unknown option for `crudo plugins list`: {filter}"),
                            "unexpected": filter,
                            "hint": "Usage: crudo plugins list [<filter>]\nFilters are id substrings, not flags.",
                            "exit_code": 1,
                        });
                    ...
                    return Err(format!(
                        "unknown option for `crudo plugins list`: {filter}\nUsage: crudo plugins list [<filter>]\nFilters are id substrings, not flags."
                    ).into());
                }
            }
        }
            ...
            CliOutputFormat::Text => {
                ...
                if matches!(action_str, "show" | "info" | "describe") {
                    if let Some(name) = target {
                        ...
                        if !found {
                            return Err(format!(
                                "plugin_not_found: plugin '{}' not found\nRun `crudo plugins list` to see available plugins.",
                                name
                            ).into());
                        }
                    }
            ...
            CliOutputFormat::Json => {
                ...
                if matches!(action_str, "help" | "-h" | "--help") {
                    ...
                    let obj = json!({
                        ...
                        "unexpected": null,
                        "usage": {
                            "direct_cli": "crudo plugins [list|show <id>|install <id>|enable <id>|disable <id>|uninstall <id>|update <id>|help]",
                    ...
                    if let Some(name) = target {
                        if filtered_plugins.is_empty() {
                            let obj = json!({
                                ...
                                "hint": "Run `crudo plugins list` to see available plugins.",
                            });
                            println!("{}", serde_json::to_string_pretty(&obj)?);
...
fn run_resumed_session_command(
    ...
    match action {
        ...
        Some("exists") => {
            let Some(target) = target else {
                return Err("/session exists requires a session id.\nUsage: crudo --resume <session> /session exists <session-id>".into());
            };
        ...
        Some("delete") => {
            let Some(target) = target else {
                return Err("/session delete requires a session id.\nUsage: crudo --resume <session> /session delete <session-id> --force".into());
            };
            Ok(ResumeCommandOutcome {
                session: session.clone(),
                message: Some(format!(
        ...
        Some(switch_or_fork @ ("switch" | "fork")) => Ok(ResumeCommandOutcome {
            ...
            json: Some(serde_json::json!({
                ...
                "hint": format!("Start a new crudo session and use /session {switch_or_fork} <id> interactively"),
            })),
        }),
...
/// `crudo session list --output-format json` should work in CI/offline.
...
fn render_repl_help() -> String {
    [
        ...
        "  Auto-save            .crudo/sessions/<workspace-fingerprint>/<session-id>.jsonl"
            .to_string(),
...
fn status_json_value(
    ...
    // #143: top-level `status` marker so crudos can distinguish
    ...
    // `classify_error_kind` so downstream crudos can switch on it directly.
    ...
    json!({
        ...
        "workspace": {
            ...
            "session_id": context.session_path.as_ref().and_then(|path| {
                ...
                // .crudo/sessions/. Extract the stem (drop the .jsonl extension).
...
fn status_context(
    ...
    // `crudo doctor` already does this; `crudo status` now matches that contract
    ...
    let (loaded_config_files, sandbox_status, config_load_error, config_load_error_kind) =
        match runtime_config.as_ref() {
            ...
            Err(err) => {
                ...
                (
                    0,
                    // Fall back to defaults for sandbox resolution so crudos still see
...
fn format_status_report(
    ...
    if let Some(err) = context.config_load_error.as_deref() {
        blocks.push(format!(
            "Config load error\n  Status           fail\n  Summary          runtime config failed to load; reporting partial status\n  Details          {err}\n  Hint             `crudo doctor` classifies config parse errors; fix the listed field and rerun"
        ));
    }
...
fn render_help_topic(topic: LocalHelpTopic) -> String {
  ...
  Usage            crudo status [--output-format <format>]
  Purpose          show the local workspace snapshot without entering the REPL
  Output           model, permissions, git state, config files, and sandbox status
  Formats          text (default), json
  Related          /status · crudo --resume latest /status"
            .to_string(),
        LocalHelpTopic::Sandbox => "Sandbox
  Usage            crudo sandbox [--output-format <format>]
  Purpose          inspect the resolved sandbox and isolation state for the current directory
  Output           namespace, network, filesystem, and fallback details
  Formats          text (default), json
  Related          /sandbox · crudo status"
            .to_string(),
        LocalHelpTopic::Doctor => "Doctor
  Usage            crudo doctor [--output-format <format>]
  Purpose          diagnose local auth, config, workspace, sandbox, and build metadata
  Output           local-only health report; no provider request or session resume required
  Formats          text (default), json
  Related          /doctor · crudo --resume latest /doctor"
            .to_string(),
        LocalHelpTopic::Acp => "ACP / Zed
  Usage            crudo acp [serve] [--output-format <format>]
  Aliases          crudo --acp · crudo -acp
  ...
  Formats          text (default), json
  Related          ROADMAP #64a (discoverability) · ROADMAP #76 (real ACP support) · crudo --help"
            .to_string(),
        LocalHelpTopic::Init => "Init
  Usage            crudo init [--output-format <format>]
  Purpose          create .crudo/settings.json, .crudo.json, .gitignore, and CLAUDE.md in the current project
  Output           per-artifact created/updated/partial/deferred/skipped status (idempotent: safe to re-run)
  Formats          text (default), json
  Related          crudo status · crudo doctor"
            .to_string(),
        LocalHelpTopic::State => "State
  Usage            crudo state [--output-format <format>]
  Purpose          read .crudo/worker-state.json written by the interactive REPL or a one-shot prompt
  Output           worker id, model, permissions, session reference (text or json)
  Formats          text (default), json
  Produces state   `crudo` (interactive REPL) or `crudo prompt <text>` (one non-interactive turn)
  Observes state   `crudo state` reads; crudohip/CI may poll this file without HTTP
  Exit codes       0 if state file exists and parses; 1 with actionable hint otherwise
  Related          crudo status · ROADMAP #139 (this worker-concept contract)"
            .to_string(),
        LocalHelpTopic::Resume => format!(
            "Resume\n  Usage            crudo resume [session-path|session-id|{LATEST_SESSION_REFERENCE}] [/slash-command ...] [--output-format <format>]\n  Alias            crudo --resume [session-path|session-id|{LATEST_SESSION_REFERENCE}]\n  Purpose          restore or inspect a saved session without starting a new provider turn\n  Output           session restore or resume-safe command output; missing sessions return session_not_found\n  Formats          text (default), json\n  Related          /resume · /session list · crudo --resume {LATEST_SESSION_REFERENCE} /status"
        ),
        LocalHelpTopic::Session => "Session
  Usage            crudo session --help [--output-format <format>]
  ...
  Direct use       run /session in the REPL or crudo --resume SESSION.jsonl /session <action>
  Formats          text (default), json
  Related          crudo resume · crudo export · .crudo/sessions/"
            .to_string(),
        LocalHelpTopic::Compact => "Compact
  Usage            crudo compact --help [--output-format <format>]
  Purpose          show compaction guidance without loading config, credentials, or a session
  Direct use       run /compact in the REPL or crudo --resume SESSION.jsonl /compact
  Output           compaction removes older tool-detail messages when the selected session is large enough
  Formats          text (default), json
  Related          crudo resume · /compact · /status"
            .to_string(),
        LocalHelpTopic::Export => "Export
  Usage            crudo export [--session <id|latest>] [--output <path>] [--output-format <format>]
  Purpose          serialize a managed session to JSON for review, transfer, or archival
  Defaults         --session latest (most recent managed session in .crudo/sessions/)
  Formats          text (default), json
  Related          /session list · crudo --resume latest"
            .to_string(),
        LocalHelpTopic::Version => "Version
  Usage            crudo version [--output-format <format>]
  Aliases          crudo --version · crudo -V
  Purpose          print the crudo CLI version and build metadata
  Formats          text (default), json
  Related          crudo doctor (full build/auth/config diagnostic)"
            .to_string(),
        LocalHelpTopic::SystemPrompt => "System Prompt
  Usage            crudo system-prompt [--cwd <path>] [--date YYYY-MM-DD] [--output-format <format>]
  Purpose          render the resolved system prompt that `crudo` would send for the given cwd + date
  Options          --cwd overrides the workspace dir · --date injects a deterministic date stamp
  Formats          text (default), json
  Related          crudo doctor · crudo dump-manifests"
            .to_string(),
        LocalHelpTopic::DumpManifests => "Dump Manifests
  Usage            crudo dump-manifests [--manifests-dir <path>] [--output-format <format>]
  ...
  Formats          text (default), json
  Related          crudo skills · crudo agents · crudo doctor"
            .to_string(),
        LocalHelpTopic::BootstrapPlan => "Bootstrap Plan
  Usage            crudo bootstrap-plan [--output-format <format>]
  ...
  Formats          text (default), json
  Related          crudo doctor · crudo status"
            .to_string(),
  ...
  Usage            crudo plugins [list|show <name>|install <path>|enable <name>|disable <name>|uninstall <name>]
  Purpose          manage lifecycle of plugins that extend tool and hook capabilities
  Formats          text (default), json
  Related          /plugins · crudo plugins --help"
            .to_string(),
        LocalHelpTopic::Mcp => "MCP Servers
  Usage            crudo mcp [list|show <server>] [--output-format <format>]
  Purpose          inspect configured MCP servers and their connection status
  Formats          text (default), json
  Related          /mcp · crudo mcp list"
            .to_string(),
        LocalHelpTopic::Config => "Config
  Usage            crudo config [section] [--output-format <format>]
  Purpose          show effective runtime configuration (model, hooks, plugins, env)
  Formats          text (default), json
  Related          /config · crudo doctor"
            .to_string(),
        LocalHelpTopic::Model => "Models
  Usage            crudo models [help] [--output-format <format>]
  Aliases          crudo model
  ...
  Formats          text (default), json
  Related          /model · crudo config model · crudo status"
            .to_string(),
        LocalHelpTopic::Settings => "Settings
  Usage            crudo settings [help] [--output-format <format>]
  Purpose          show effective settings/config using the local config envelope
  Output           same as crudo config settings; no provider request or session resume required
  Formats          text (default), json
  Related          crudo config · crudo doctor"
            .to_string(),
        LocalHelpTopic::Diff => "Diff
  Usage            crudo diff [--output-format <format>]
  ...
  Output           writes provider settings to ~/.crudo/settings.json (0600 permissions)
  Related          /model · /config · crudo doctor"
            .to_string(),
    }
}
...
fn print_models(
    ...
    match output_format {
        CliOutputFormat::Text => {
            ...
            println!("  Usage            crudo --model <provider/model> prompt <text>");
        }
        CliOutputFormat::Json => {
            println!(
                "{}",
                serde_json::to_string_pretty(&json!({
                    ...
                    "message": "Use --model <provider/model> or configure a model in crudo settings."
                }))?
            );
        }
    }
...
fn acp_status_message() -> &'static str {
    "ACP/Zed editor integration is not implemented in crudo yet. `crudo acp serve` reports status only and does not launch a daemon or JSON-RPC endpoint. Use the normal terminal surfaces for now."
}
...
fn acp_status_json() -> serde_json::Value {
    json!({
        ...
        "contracts": {
            ...
            "stable_status_surface": "crudo acp [serve] --output-format json",
...
fn print_acp_status(output_format: CliOutputFormat) -> Result<(), Box<dyn std::error::Error>> {
    match output_format {
        CliOutputFormat::Text => {
            println!(
                "ACP / Zed\n  Status           not implemented\n  Launch           `crudo acp serve` reports status only; no editor daemon or JSON-RPC endpoint is available yet\n  Today            use `crudo prompt`, the REPL, or `crudo doctor` for local verification\n  Message          {}",
                acp_status_message()
            );
        }
        CliOutputFormat::Json => {
        ...
        let section_rendered: Option<String> = match section {
            ...
            other => {
                ...
                let hint = if matches!(other, "list" | "show" | "info") {
                    format!(
                        "'crudo config {other}' is not a subcommand. To list all config: `crudo config`. To inspect a section: `crudo config <section>` where section is one of: env, hooks, model, plugins, mcp, sandbox, permissions, skills, agents, settings."
                    )
                } else {
                    format!(
...
fn render_memory_report() -> Result<String, Box<dyn std::error::Error>> {
  ...
  Instruction files {}",
    ...
    if project_context.instruction_files.is_empty() {
        ...
        lines.push(
            "  No CLAUDE.md, CRUDO.md, AGENTS.md, or scoped instruction files discovered in the current directory ancestry."
                .to_string(),
        );
    } else {
        lines.push("Discovered files".to_string());
...
/// string so crudos can detect per-artifact state without substring matching.
fn init_json_value(report: &crate::init::InitReport, message: &str) -> serde_json::Value {
    ...
    let hint = if already_initialized {
        "Workspace already initialised. Run `crudo doctor` to verify health, or edit CLAUDE.md to customise guidance."
    } else {
        "Review and tailor CLAUDE.md to your project, then run `crudo doctor` to verify the workspace."
    };
    json!({
        "kind": "init",
        "action": "init",
...
fn plugins_command_payload_from_result(
    ...
    let message = match config_load_error.as_deref() {
        ...
        None if mcp_validation.has_invalid_servers() => format!(
            "MCP validation\n  Status           warn\n  Summary          {} MCP server entries are invalid; reporting plugins with valid MCP siblings only\n  Hint             Inspect `crudo mcp list --output-format json` invalid_servers and fix each rejected mcpServers entry.\n\n{}",
...
impl AnthropicRuntimeClient {
    fn new(
        ...
        // (`crates/crudo-cli/tests/compact_output.rs`) to point
        // crudo at its fake Anthropic endpoint. We also attach a
...
fn format_context_window_blocked_error(session_id: &str, error: &api::ApiError) -> String {
    ...
    lines.push(format!(
        "  Resume compact   crudo --resume {session_id} /compact"
    ));
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    writeln!(out, "crudo v{VERSION}")?;
    writeln!(out)?;
    writeln!(out, "Usage:")?;
    writeln!(
        out,
        "  crudo [--model MODEL] [--allowedTools TOOL[,TOOL...]]"
    )?;
    writeln!(out, "      Start the interactive REPL")?;
    writeln!(
        out,
        "  crudo [--model MODEL] [--output-format text|json] prompt [--stdin] [TEXT]"
    )?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  crudo [--model MODEL] [--output-format text|json] TEXT"
    )?;
    ...
    writeln!(
        out,
        "  crudo --resume [SESSION.jsonl|session-id|latest] [/status] [/compact] [...]"
    )?;
    writeln!(
        out,
        "      Inspect or maintain a saved session without entering the REPL"
    )?;
    writeln!(out, "  crudo help")?;
    writeln!(out, "      Alias for --help")?;
    writeln!(out, "  crudo version")?;
    writeln!(out, "      Alias for --version")?;
    writeln!(out, "  crudo status")?;
    writeln!(
        out,
        "      Show the current local workspace status snapshot"
    )?;
    writeln!(out, "  crudo sandbox")?;
    writeln!(out, "      Show the current sandbox isolation snapshot")?;
    writeln!(out, "  crudo doctor")?;
    writeln!(
        out,
        "      Diagnose local auth, config, workspace, and sandbox health"
    )?;
    writeln!(out, "  crudo acp [serve]")?;
    writeln!(
        out,
    ...
    writeln!(out, "  crudo dump-manifests [--manifests-dir PATH]")?;
    writeln!(out, "  crudo bootstrap-plan")?;
    ...
    writeln!(
        out,
        "  crudo export [PATH] [--session SESSION] [--output PATH]"
    )?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "                              CRUDO_OUTPUT_FORMAT sets the default; flags override env"
    )?;
    writeln!(
        out,
        "                              Log env vars: CRUDO_LOG or RUST_LOG"
    )?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  REPL turns auto-save to .crudo/sessions/<session-id>.{PRIMARY_SESSION_EXTENSION}"
    )?;
    writeln!(
        out,
    ...
    writeln!(out, "  crudo --model claude-opus \"summarize this repo\"")?;
    writeln!(
        out,
        "  crudo --output-format json prompt \"explain src/main.rs\""
    )?;
    writeln!(out, "  crudo --compact \"summarize Cargo.toml\" | wc -l")?;
    writeln!(
        out,
        "  crudo --allowedTools read,glob \"summarize Cargo.toml\""
    )?;
    writeln!(out, "  crudo --resume {LATEST_SESSION_REFERENCE}")?;
    writeln!(
        out,
        "  crudo --resume {LATEST_SESSION_REFERENCE} /status /diff /export notes.txt"
    )?;
    writeln!(out, "  crudo agents")?;
    writeln!(out, "  crudo mcp show my-server")?;
    writeln!(out, "  crudo /skills")?;
    writeln!(out, "  crudo doctor")?;
    ...
    writeln!(out, "  crudo init")?;
    writeln!(out, "  crudo export")?;
    writeln!(out, "  crudo export conversation.md")?;
    Ok(())
}
...
mod tests {
    ...
    fn context_window_preflight_errors_render_recovery_steps() {
        ...
        assert!(
            rendered.contains("Resume compact   crudo --resume session-issue-32 /compact"),
            "{rendered}"
        );
        assert!(
    ...
    fn temp_dir() -> PathBuf {
        ...
        std::env::temp_dir().join(format!("crudo-cli-{nanos}-{unique}"))
    }
    ...
    fn default_permission_mode_uses_project_config_when_env_is_unset() {
        ...
        std::fs::create_dir_all(cwd.join(".crudo")).expect("project config dir should exist");
        std::fs::create_dir_all(&config_home).expect("config home should exist");
        std::fs::write(
            cwd.join(".crudo").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
        .expect("project config should write");

        let original_config_home = std::env::var("CRUDO_CONFIG_HOME").ok();
        let original_permission_mode = std::env::var("RUSTY_CLAUDE_PERMISSION_MODE").ok();
        std::env::set_var("CRUDO_CONFIG_HOME", &config_home);
        std::env::remove_var("RUSTY_CLAUDE_PERMISSION_MODE");

        let resolved = with_current_dir(&cwd, super::default_permission_mode);

        match original_config_home {
            Some(value) => std::env::set_var("CRUDO_CONFIG_HOME", value),
            None => std::env::remove_var("CRUDO_CONFIG_HOME"),
        }
    ...
    fn export_help_json_is_bounded_and_parseable_384() {
        ...
        assert_eq!(
            value["usage"],
            "crudo export [--session <id|latest>] [--output <path>] [--output-format <format>]"
        );
...
"#,
        ...
        .expect("write malformed .crudo.json");

        let previous_config_home = std::env::var("CRUDO_CONFIG_HOME").ok();
        std::env::set_var("CRUDO_CONFIG_HOME", &config_home);
    ...
    fn status_degrades_gracefully_on_malformed_mcp_config_143() {
        // #143: previously `crudo status` hard-failed on any config parse error,
        // taking down the entire health surface for one malformed MCP entry.
        // `crudo doctor` already degrades gracefully; this test locks `status`
...
"#,
    ...
    fn state_error_surfaces_actionable_worker_commands_139() {
        // #139: the error for missing `.crudo/worker-state.json` must name
        // the concrete commands that produce worker state, otherwise crudos
        ...
        assert!(
            message.contains("crudo prompt"),
            "error should name `crudo prompt <text>` as a producer: {message}"
        );
        assert!(
        ...
        // so crudos can discover the contract without hitting the error first.
        ...
        assert!(
            state_help.contains("crudo prompt"),
            "state help must name `crudo prompt <text>` as a producer: {state_help}"
        );
    }
    ...
    fn classify_error_kind_returns_correct_discriminants() {
        ...
        assert_eq!(
            classify_error_kind("unsupported ACP invocation. Use `crudo acp`."),
            "unsupported_acp_invocation"
        );
        assert_eq!(
        ...
        assert_eq!(
            classify_error_kind("/tmp/.crudo/settings.json: expected ',', found end of input"),
            "config_parse_error"
        );
        assert_eq!(
            classify_error_kind(
                "/path/to/.crudo.json: field \"model\" must be a string, got a number"
            ),
            "config_parse_error"
        );
        ...
        assert_eq!(
            classify_error_kind(
                "unexpected extra arguments after `crudo diff`: --bogus\nUsage: crudo diff"
            ),
            "unexpected_extra_args"
        );
        assert_eq!(
            classify_error_kind(
                "`crudo logout` has been removed.\nSet ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN instead."
            ),
            "removed_subcommand"
        );
    ...
    fn punctuation_bearing_single_token_still_dispatches_to_prompt() {
        ...
        // doesn't pick up a stale .crudo/settings.json from other tests that
    ...
    fn rejects_unknown_options_with_helpful_guidance() {
        ...
        assert!(error.contains("unknown option: --resum"));
        assert!(error.contains("Did you mean --resume?"));
        assert!(error.contains("crudo --help"));
    }
    ...
    fn resolve_repl_model_falls_back_to_anthropic_model_env_when_default() {
        ...
        std::env::remove_var("ANTHROPIC_MODEL");
        std::env::remove_var("CRUDO_CONFIG_HOME");
        fs::remove_dir_all(root).expect("cleanup temp dir");
    }
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("crudo help"));
        assert!(help.contains("crudo version"));
        assert!(help.contains("crudo status"));
        assert!(help.contains("crudo sandbox"));
        assert!(help.contains("crudo init"));
        assert!(help.contains("crudo acp [serve]"));
        assert!(help.contains("crudo agents"));
        assert!(help.contains("crudo mcp"));
        assert!(help.contains("crudo skills"));
        assert!(help.contains("crudo /skills"));
        assert!(help.contains("ultraworkers/crudo"));
        assert!(help.contains("cargo install crudo"));
        assert!(!help.contains("crudo login"));
        assert!(!help.contains("crudo logout"));
    }
    ...
    fn session_lifecycle_prefers_running_process_over_idle_shell() {
        ...
        let lifecycle = classify_session_lifecycle_from_panes(
            ...
            vec![
                ...
                TmuxPaneSnapshot {
                    pane_id: "%2".to_string(),
                    current_command: "crudo".to_string(),
        ...
        assert_eq!(lifecycle.pane_command.as_deref(), Some("crudo"));
    ...
    fn session_list_surfaces_saved_dirty_abandoned_lifecycle() {
        ...
        git(&["config", "user.email", "tests@example.com"], &workspace);
        git(&["config", "user.name", "Rusty Claude Tests"], &workspace);
        fs::write(workspace.join(".gitignore"), ".crudo/\n").expect("write gitignore");
        fs::write(workspace.join("tracked.txt"), "hello\n").expect("write tracked");
    ...
    #[test]
    fn status_json_surfaces_session_lifecycle_for_crudohip() {
        let context = super::StatusContext {
            cwd: PathBuf::from("/tmp/project"),
            session_path: None,
            loaded_config_files: 0,
            ...
            session_lifecycle: SessionLifecycleSummary {
                ...
                pane_command: Some("crudo".to_string()),
                pane_path: Some(PathBuf::from("/tmp/project")),
                workspace_dirty: false,
                abandoned: false,
                all_panes: vec![],
    ...
    fn boot_preflight_snapshot_reports_machine_readable_contract_fields() {
        ...
        git(&["config", "user.name", "Rusty Claude Tests"], &workspace);
        fs::write(workspace.join("tracked.txt"), "hello\n").expect("write tracked");
        fs::write(workspace.join(".crudo.json"), r#"{"trustedRoots": ["."]}"#)
            .expect("write config");
        git(&["add", "tracked.txt"], &workspace);
        git(&["commit", "-m", "init", "--quiet"], &workspace);
...
UU conflicted.rs",
    ...
    fn help_mentions_jsonl_resume_examples() {
        ...
        print_help_to(&mut help).expect("help should render");
        let help = String::from_utf8(help).expect("help should be utf8");
        assert!(help.contains("crudo --resume [SESSION.jsonl|session-id|latest]"));
        assert!(help.contains("Use `latest` with --resume, /resume, or /session switch"));
        assert!(help.contains("crudo --resume latest"));
        assert!(help.contains("crudo --resume latest /status /diff /export notes.txt"));
    }
    ...
    fn managed_sessions_default_to_jsonl_and_resolve_legacy_json() {
        ...
        let legacy_path = workspace.join(".crudo/sessions/legacy.json");
    ...
    fn load_session_reference_rejects_workspace_mismatch() {
        ...
        let session_path = workspace_a.join(".crudo/sessions/legacy-cross.jsonl");
        std::fs::create_dir_all(
            session_path
                .parent()
    ...
    fn resume_usage_mentions_latest_shortcut() {
        ...
        assert!(usage.contains(".crudo/sessions/<workspace-fingerprint>/<session-id>.jsonl"));
    ...
    fn temp_workspace(label: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("crudo-cli-{label}-{nanos}"))
    }
    ...
    fn ultraplan_progress_lines_include_phase_step_and_elapsed_status() {
        let snapshot = InternalPromptProgressState {
            ...
            detail: Some("reading rust/crates/crudo-cli/src/main.rs".to_string()),
            saw_final_text: false,
        };
    ...
    fn describe_tool_progress_summarizes_known_tools() {
        ...
        assert!(
            describe_tool_progress("bash", r#"{"command":"cargo test -p crudo-cli"}"#)
                .contains("cargo test -p crudo-cli")
        );
        assert_eq!(
...
mod dump_manifests_tests {
    ...
    fn dump_manifests_defaults_to_rust_resolver_inventory() {
        let root =
            std::env::temp_dir().join(format!("crudo_test_rust_manifests_{}", std::process::id()));
    ...
    fn dump_manifests_scopes_explicit_manifest_dir_without_upstream_ts() {
        let root = std::env::temp_dir().join(format!(
            "crudo_test_explicit_manifest_dir_{}",
            std::process::id()
        ));
        let workspace = root.join("workspace");
    ...
    fn dump_manifests_missing_explicit_dir_has_typed_kind() {
        let root = std::env::temp_dir().join(format!(
            "crudo_test_missing_manifest_dir_{}",
            std::process::id()
        ));
        let workspace = root.join("workspace");

...
fn upstream_repo_candidates(primary_repo_root: &Path) -> Vec<PathBuf> {
    ...
    for ancestor in primary_repo_root.ancestors().take(4) {
        candidates.push(ancestor.join("crudo"));
        candidates.push(ancestor.join("crudo-code"));
    }

    candidates.push(primary_repo_root.join("reference-source").join("crudo"));
    candidates.push(primary_repo_root.join("vendor").join("crudo"));

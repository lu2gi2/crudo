...
fn discover_instruction_files(
    ...
    for dir in directories {
        for candidate in [
            ...
            dir.join(".crudo").join("CLAUDE.md"),
            dir.join(".claude").join("CLAUDE.md"),
            dir.join(".crudo").join("instructions.md"),
        ] {
            push_context_file(&mut files, candidate)?;
        }
        push_rules_dir(&mut files, dir.join(".crudo").join("rules"))?;
        push_rules_dir(&mut files, dir.join(".crudo").join("rules.local"))?;
...
fn render_config_section(config: &RuntimeConfig) -> String {
    ...
    if config.loaded_entries().is_empty() {
        lines.extend(prepend_bullets(vec![
            "No Crudo settings files loaded.".to_string()
        ]));
        return lines.join("\n");
    }
...
mod tests {
    ...
    #[test]
    fn discovers_crudo_rules_files_in_sorted_order() {
        let root = temp_dir();
        let rules = root.join(".crudo").join("rules");
        let local_rules = root.join(".crudo").join("rules.local");
    ...
    fn rules_import_none_suppresses_external_framework_rules() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".crudo").join("rules")).expect("rules dir");
        fs::write(
            root.join(".crudo").join("rules").join("project.md"),
            "crudo rule",
        )
        .expect("write crudo rule");
        ...
        assert!(rendered.contains("crudo rule"));
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".crudo")).expect("nested crudo dir");
        fs::create_dir(root.join(".git")).expect("git boundary");
        fs::write(root.join("CLAUDE.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("CLAUDE.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".crudo")).expect("apps crudo dir");
        fs::write(root.join("apps").join("CLAUDE.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(
            root.join("apps").join(".crudo").join("instructions.md"),
            "apps dot claude instructions",
        )
        .expect("write apps dot claude instructions");
        fs::write(nested.join(".crudo").join("CLAUDE.md"), "nested rules")
            .expect("write nested rules");
        fs::write(
            nested.join(".crudo").join("instructions.md"),
            "nested instructions",
        )
    ...
    #[test]
    fn discovers_claude_crudo_agents_and_dot_claude_instruction_files_together() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".claude")).expect("dot claude dir");
        fs::write(root.join("CLAUDE.md"), "claude instructions").expect("write CLAUDE.md");
        fs::write(root.join("CRUDO.md"), "crudo instructions").expect("write CRUDO.md");
        ...
        assert_eq!(
            sources,
            vec!["claude_md", "crudo_md", "agents_md", "claude_claude_md"]
        );
        assert!(rendered.contains("claude instructions"));
        assert!(rendered.contains("crudo instructions"));
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.crudo/CLAUDE.md")),
            "CLAUDE.md"
        );
    }
    ...
    fn load_system_prompt_reads_claude_files_and_config() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".crudo")).expect("crudo dir");
        ...
        let original_crudo_home = std::env::var("CRUDO_CONFIG_HOME").ok();
        std::env::set_var("HOME", &root);
        std::env::set_var("CRUDO_CONFIG_HOME", root.join("missing-home"));
...
",
        ...
        if let Some(value) = original_crudo_home {
            std::env::set_var("CRUDO_CONFIG_HOME", value);
        } else {
    ...
    fn discovers_dot_claude_instructions_markdown() {
        ...
        assert!(context
            .instruction_files
            .iter()
            .any(|file| file.path.ends_with(".crudo/instructions.md")));
        assert!(

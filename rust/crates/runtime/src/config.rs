...
pub const CRUDO_SETTINGS_SCHEMA_NAME: &str = "SettingsSchema";
...
pub enum RulesImportConfig {
    ...
    #[default]
    Auto,
    /// Do not import external framework rules; keep Crudo instruction files only.
    None,
...
///
/// Represents the `provider` section in `~/.crudo/settings.json`, used as a
...
/// OAuth client configuration used by the main Crudo runtime.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".crudo.json"),
            |parent| parent.join(".crudo.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".crudo.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".crudo").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".crudo").join("settings.local.json"),
            },
        ]
    }
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("CRUDO_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".crudo")))
        .unwrap_or_else(|| PathBuf::from(".crudo"))
}

/// Save provider settings to the user-level `~/.crudo/settings.json`.
...
/// Remove the `provider` section from the user-level `~/.crudo/settings.json`.
...
fn read_optional_json_object(path: &Path) -> Result<OptionalConfigFile, ConfigError> {
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".crudo.json");
...
mod tests {
    use super::{
        ...
        RuntimeHookCommand, RuntimeHookConfig, RuntimePluginConfig, CRUDO_SETTINGS_SCHEMA_NAME,
    };
    use crate::json::JsonValue;
    ...
    fn rejects_non_object_settings_files() {
        ...
        let home = root.join("home").join(".crudo");
    ...
    fn loads_and_merges_claude_code_config_files_by_precedence() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".crudo");
        fs::create_dir_all(cwd.join(".crudo")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".crudo.json"),
        ...
        assert_eq!(CRUDO_SETTINGS_SCHEMA_NAME, "SettingsSchema");
    ...
    fn inspect_classifies_missing_loaded_and_legacy_skipped_files() {
        ...
        fs::create_dir_all(cwd.join(".crudo")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");
        fs::write(cwd.join(".crudo.json"), "{not json").expect("write legacy config");
        fs::write(
            cwd.join(".crudo").join("settings.json"),
            r#"{"model":"opus"}"#,
        )
    ...
    fn inspect_reports_parse_errors_but_keeps_valid_merged_config() {
        ...
        fs::write(home.join("settings.json"), r#"{"model":"sonnet"}"#)
            .expect("write user settings");
        fs::write(cwd.join(".crudo").join("settings.json"), "{not json")
    ...
    fn loads_valid_hook_entries_and_records_invalid_siblings_441() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".crudo");
        let project_settings = cwd.join(".crudo").join("settings.json");
        fs::create_dir_all(cwd.join(".crudo")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

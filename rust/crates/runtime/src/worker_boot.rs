...
/// Write current worker state to `.crudo/worker-state.json` under the worker's cwd.
/// This is the file-based observability surface: external observers (crudohip, orchestrators)
...
struct StateSnapshot<'a> {
    ...
    updated_at: u64,
    /// Seconds since last state transition. Crudohip uses this to detect
    /// stalled workers without computing epoch deltas.
    seconds_since_update: u64,
}

fn emit_state_file(worker: &Worker) {
    let state_dir = std::path::Path::new(&worker.cwd).join(".crudo");
...
mod tests {
    ...
    fn startup_timeout_classifies_tool_permission_prompt() {
        ...
        let timed_out = registry
            .observe_startup_timeout(&worker.worker_id, "crudo prompt", true, true)
    ...
    fn emit_state_file_writes_worker_status_on_transition() {
        ...
        let state_path = cwd_path.join(".crudo").join("worker-state.json");
        assert!(
            state_path.exists(),
    ...
    fn startup_timeout_preserves_original_prompt_sent_timestamp() {
        ...
        let timed_out = registry
            .observe_startup_timeout(&worker.worker_id, "crudo worker", true, true)
            .expect("startup timeout observe should succeed");

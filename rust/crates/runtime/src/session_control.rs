...
pub struct SessionStore {
    ...
    /// `/home/user/project/.crudo/sessions/a1b2c3d4e5f60718/`.
...
impl SessionStore {
    ...
    ///
    /// The on-disk layout is `<cwd>/.crudo/sessions/<workspace_hash>/`,
    ...
    pub fn latest_session_excluding(
        ...
        // Fallback: scan all workspace namespaces under ~/.crudo/sessions/
        // and project-local .crudo/sessions/ so /resume latest finds sessions
    ...
    /// Checks both the global root (~/.crudo/sessions/) and the project-local
    /// .crudo/sessions/ parent directory. Used as a fallback when the current
    /// workspace has no sessions.
    #[allow(clippy::unnecessary_wraps)]
    fn scan_global_sessions(&self) -> Result<Vec<ManagedSessionSummary>, SessionControlError> {
        let mut sessions = Vec::new();

        // Scan global root: ~/.crudo/sessions/<fingerprint>/
        ...
        // Scan project-local parent: <cwd>/.crudo/sessions/<fingerprint>/
...
/// Points to `~/.crudo/sessions/` (or `$CRUDO_CONFIG_HOME/sessions/`).
#[must_use]
...
fn format_missing_session_reference(reference: &str, sessions_root: &Path) -> String {
    // #80: show the actual workspace-fingerprint directory instead of lying about .crudo/sessions/
    let fingerprint_dir = sessions_root
        .file_name()
        .and_then(|f| f.to_str())
        .unwrap_or("<unknown>");
    format!(
        "session not found: {reference}\nHint: managed sessions live in .crudo/sessions/{fingerprint_dir}/ (workspace-specific partition).\nTry `{LATEST_SESSION_REFERENCE}` for the most recent session or `/session list` in the REPL."
    )
}
...
fn format_no_managed_sessions(sessions_root: &Path) -> String {
    ...
    let fingerprint_dir = sessions_root
        ...
        .unwrap_or("<unknown>");
    format!(
        "no managed sessions found in .crudo/sessions/{fingerprint_dir}/\nStart `crudo` to create a session, then rerun with `--resume {LATEST_SESSION_REFERENCE}`.\nNote: /resume {LATEST_SESSION_REFERENCE} searches all workspaces."
    )
}
...
fn format_all_sessions_empty(sessions_root: &Path) -> String {
    let fingerprint_dir = sessions_root
        ...
        .unwrap_or("<unknown>");
    format!(
        "all sessions are empty (0 messages) in .crudo/sessions/{fingerprint_dir}/\nThis usually means a fresh `crudo` session is running but no messages have been sent yet.\nWait for a response in your other session, then try `--resume {LATEST_SESSION_REFERENCE}` again."
    )
}
...
mod tests {
    ...
    fn session_store_from_cwd_is_side_effect_free_until_save() {
        ...
        // then — resolving the store must not create .crudo/session partitions.
        assert!(
            !workspace.join(".crudo").exists(),
            "session store construction must not create .crudo side effects"
        );
        assert!(
    ...
    fn session_store_rejects_legacy_session_from_other_workspace() {
        ...
        let store_b = SessionStore::from_cwd(&workspace_b).expect("store b should build");
        let legacy_root = workspace_b.join(".crudo").join("sessions");
    ...
    fn session_store_loads_safe_legacy_session_from_same_workspace() {
        ...
        let store = SessionStore::from_cwd(&base).expect("store should build");
        let legacy_root = base.join(".crudo").join("sessions");
    ...
    fn latest_session_returns_all_empty_error_when_sessions_exist_but_have_no_messages() {
        ...
        let _crudo_config_home = EnvVarGuard::set("CRUDO_CONFIG_HOME", &isolated_config_home);
        let store = SessionStore::from_cwd(&base).expect("store should build");

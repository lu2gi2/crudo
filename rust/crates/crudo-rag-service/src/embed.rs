...
impl EmbedConfig {
    pub fn from_env() -> Result<Self, String> {
        let api_key = std::env::var("CRUDO_RAG_OPENAI_API_KEY")
            .or_else(|_| std::env::var("OPENAI_API_KEY"))
            .map_err(|_| {
                "set CRUDO_RAG_OPENAI_API_KEY or OPENAI_API_KEY for embeddings".to_string()
            })?;
        let base_url = std::env::var("CRUDO_RAG_EMBEDDING_BASE_URL")
            .unwrap_or_else(|_| "https://api.openai.com/v1".into());
        let model = std::env::var("CRUDO_RAG_EMBEDDING_MODEL")
    ...
    pub fn mock_from_env() -> Option<Self> {
        if std::env::var("CRUDO_RAG_MOCK_PROVIDERS").ok().as_deref() != Some("1") {
            return None;
        }
        Some(Self {
            api_key: "mock".into(),

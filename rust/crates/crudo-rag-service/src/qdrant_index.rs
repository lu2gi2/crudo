...
impl QdrantConfig {
    pub fn from_env() -> Option<Self> {
        let url = std::env::var("CRUDO_RAG_QDRANT_URL").ok()?;
        let collection = std::env::var("CRUDO_RAG_QDRANT_COLLECTION")
            .ok()
            .unwrap_or_else(|| "crudo_rag_chunks".to_string());
        let api_key = std::env::var("CRUDO_RAG_QDRANT_API_KEY").ok();
        Some(Self {
            url,
            api_key,
            collection,

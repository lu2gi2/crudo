//! `crudo-rag-service` — HTTP API + `ingest` subcommand.
...
#[derive(Parser)]
struct ServeArgs {
    #[arg(long, env = "CRUDO_RAG_DB", default_value = ".crudo-rag/index.sqlite")]
    db: PathBuf,
}
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let db = if let Some(Cmd::Serve(s)) = cli.command {
        ...
        PathBuf::from(
            std::env::var("CRUDO_RAG_DB").unwrap_or_else(|_| ".crudo-rag/index.sqlite".into()),
        )
    };
    ...
    let port: u16 = std::env::var("CRUDO_RAG_PORT")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(8787);
    let host: std::net::IpAddr = std::env::var("CRUDO_RAG_HOST")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(std::net::IpAddr::V4(std::net::Ipv4Addr::LOCALHOST));
    let addr = std::net::SocketAddr::from((host, port));
    eprintln!(
        "crudo-rag-service db={} listen=http://{addr}",

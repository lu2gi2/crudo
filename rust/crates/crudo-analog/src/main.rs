//! Binary wrapper for `crudo_analog::run` — see `how_to_run.md` in repo root.
...
#[command(
    name = "crudo-analog",
    version,
    about = "Lean tool-agent loop (read/list/grep/write) on crudo `api` providers"
)]
...
enum Commands {
    /// Verify credentials, `cargo check -p crudo-analog` (or `--release-build`), config merge preview, optional `--tcp-ping`.
...
#[derive(Subcommand, Debug)]
enum ConfigSub {
    /// Parse `.crudo-analog.toml` and profile; print a merge preview (no API calls).
...
#[derive(Parser, Debug)]
struct RunCli {
    /// Config file (default: `<workspace>/.crudo-analog.toml` if that path exists).
    ...
    /// Profile snippet TOML (`line = "..."`). Default: `~/.crudo-analog/profile.toml` if it exists.
    #[arg(long, value_name = "PATH")]
...
fn config_file_path(cli: &RunCli) -> PathBuf {
    cli.config
        .clone()
        .unwrap_or_else(|| cli.workspace.join(".crudo-analog.toml"))
}
...
fn load_file_config(path: &Path) -> AnalogFileConfig {
    ...
    match load_analog_toml(path) {
        ...
        Err(e) => {
            eprintln!(
                "[crudo-analog] warning: failed to read {}: {e}",
                path.display()
            );
...
fn merge_preset(cli: Option<PresetCli>, file: Option<&str>, prompt: &str) -> Preset {
    if let Some(p) = cli {
        return match p {
            PresetCli::Auto => crudo_analog::infer_preset_from_prompt(prompt),
            other => Preset::from(other),
        };
    }
    if file.is_some_and(|s| s.trim().eq_ignore_ascii_case("auto")) {
        return crudo_analog::infer_preset_from_prompt(prompt);
    }
    if let Some(s) = file.and_then(Preset::from_toml_str) {
        return s;
    }
    crudo_analog::infer_preset_from_prompt(prompt)
}
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    match root.command {
        ...
        Some(Commands::Complete(co)) => {
            ...
            generate(shell, &mut cmd, "crudo-analog", &mut std::io::stdout());
            return Ok(());
        }
        None => {}
    }
    ...
    if let Err(e) = crudo_analog::run(config, &mut out).await {

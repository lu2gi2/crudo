//! `crudo-analog config validate` — parse TOML and profile without calling the API.
...
pub struct ValidateCli {
    ...
    /// Require `<workspace>/.crudo-analog.toml` (or `--config`) to exist and parse.
...
pub fn run_validate(cli: ValidateCli) -> i32 {
    ...
    match &prof_path {
        None => println!(
            "Profile: (none — no CLI/TOML path and no default ~/.crudo-analog/profile.toml)"
        ),

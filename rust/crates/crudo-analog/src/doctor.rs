//! `crudo-analog doctor` — environment and Cargo sanity checks.
...
pub struct DoctorCli {
    /// Workspace root (same as `crudo-analog -w`; config defaults to `<workspace>/.crudo-analog.toml`).
    #[arg(short = 'w', long, default_value = ".", value_name = "DIR")]
    pub workspace: PathBuf,
    /// Config path (default: `<workspace>/.crudo-analog.toml`).
    ...
    /// Profile TOML path (optional; if omitted, uses TOML `profile` or default `~/.crudo-analog/profile.toml`).
    #[arg(long, value_name = "PATH")]
    ...
    #[arg(long)]
    pub no_build: bool,
    /// Run `cargo build --release -p crudo-analog` (writes `target/release/…`, safe while `cargo run` holds `target/debug/…` on Windows).
...
pub fn run_doctor(args: DoctorCli) -> i32 {
    println!("crudo-analog doctor — environment and build checks\n");
    ...
    let cfg_path = args
        .config
        .clone()
        .unwrap_or_else(|| workspace.join(".crudo-analog.toml"));
    let (file_cfg, cfg_note) = if cfg_path.is_file() {
        match load_analog_toml(&cfg_path) {
            Ok(c) => (c, "loaded"),
            Err(e) => {
                eprintln!(
                    "[crudo-analog] doctor: failed to parse {}: {e} (using empty TOML defaults)",
                    cfg_path.display()
                );
    ...
    println!("Effective config (merge of `.crudo-analog.toml` + flags below):");
...
fn resolve_profile_path_doctor(
    ...
    let def = home_dir()?.join(".crudo-analog").join("profile.toml");
    if def.is_file() {
        Some(def)
    } else {
        None
...
fn workspace_root_or_eprint(manifest_dir: Option<&Path>) -> Option<PathBuf> {
    ...
    discover_cargo_workspace(&start).or_else(|| {
        eprintln!(
            "cargo: could not find a [workspace] Cargo.toml above {}.\n      Pass --manifest-dir pointing at the `rust` folder of crudo.",
            start.display()
        );
        None
    })
            ...
            if !key.trim().is_empty() {
                ...
                if embeddings_check {
                    let model = std::env::var("OPENAI_EMBEDDING_MODEL")
                        .ok()
                        .or_else(|| std::env::var("CRUDO_RAG_EMBEDDING_MODEL").ok())

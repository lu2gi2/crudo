//! `crudo-analog agents` — run multiple specialized sub-agents sequentially.
...
fn config_path(args: &AgentsCli) -> PathBuf {
    args.config
        .clone()
        .unwrap_or_else(|| args.workspace.join(".crudo-analog.toml"))
}
...
fn ensure_base_session(base_session: &Path, workspace: &Path, prompt: &str) -> Result<(), String> {
    ...
    crudo_analog::session_save(base_session, &ws_s, &model, Preset::None, &messages)?;
    Ok(())
}
    ...
    for<'a> F: FnMut(AnalogConfig, &'a mut Vec<u8>) -> RunFuture<'a>,
{
    ...
    println!("crudo-analog agents (sequential)\n");
    ...
    for (i, spec) in specs.into_iter().enumerate() {
        ...
        let profile_hint = if let Some(ref p) = profile_path {
            crudo_analog::load_profile_hint(p).unwrap_or(None)
        } else {
            None
        };
        ...
        let cfg = AnalogConfig {
            ...
            language: file_cfg
                .language
                .as_deref()
                .and_then(crudo_analog::AnalogLanguage::from_toml_str)
                .unwrap_or_default(),
...
mod tests {
    ...
    fn runs_two_agents_sequentially_with_stub_runner() {
        ...
        let base_session = workspace.join(".crudo").join("agents-base.json");

...
/// Refuses unrestricted permission modes in non-interactive runs unless explicitly opted in (same spirit as full `crudo` CLI).
...
pub fn enforce_non_interactive_permission_rules_with_tty(
    ...
    if mode == PermissionMode::Prompt && !stdin_is_tty {
        eprintln!(
            "[crudo-analog] warning: 'prompt' without a TTY cannot confirm tool use; writes remain denied. For headless edits use --permission workspace-write."
        );
    }
    Ok(())
}
...
pub const NDJSON_SCHEMA: &str = "crudo-analog-ndjson";
...
/// Map TOML / policy strings to [`PermissionMode`] (same rules as the main `crudo-analog` CLI).
#[must_use]
...
/// Effective options after merging `.crudo-analog.toml` with optional CLI overrides (same precedence as `crudo-analog` run).
#[must_use]
pub fn resolve_analog_options(
    ...
    let (model, m_src) = if let Some(ref m) = overrides.model {
        ...
        if fm.is_empty() {
            ...
            (fm.to_string(), ".crudo-analog.toml")
        }
    } else {
    ...
    let ad_src = match (
        ...
        (true, true) => "CLI and .crudo-analog.toml",
        (true, false) => "CLI",
        (false, true) => ".crudo-analog.toml",
...
/// Match main CLI profile resolution: `--profile`, then TOML `profile`, then default `~/.crudo-analog/profile.toml` if it exists.
#[must_use]
...
pub fn session_bootstrap_messages(
    ...
    if file.workspace != workspace {
        eprintln!(
            "[crudo-analog] warning: session workspace differs (file: {}, current: {})",
            file.workspace, workspace
        );
    }
    if file.model != model {
        eprintln!(
            "[crudo-analog] warning: session model differs (file: {}, current: {})",
            file.model, model
        );
    }
    ...
    if want_preset.as_deref() != file.preset.as_deref() {
        eprintln!(
            "[crudo-analog] warning: session preset {:?} vs current {:?}",
...
fn session_warn_common() {
    eprintln!(
        "[crudo-analog] session: files may contain secrets (tool output, pasted keys). Do not share. Large histories increase API cost."
    );
}
...
/// Read `~/.crudo-analog/profile.toml`-style file: single `line` merged into system prompt.
pub fn load_profile_hint(path: &Path) -> Result<Option<String>, String> {
    ...
    if nchars > PROFILE_LINE_MAX_CHARS {
        eprintln!(
            "[crudo-analog] warning: profile line truncated ({} chars; max {})",
...
/// Optional defaults from `.crudo-analog.toml` (see `load_analog_toml`).
...
pub struct AnalogFileConfig {
    ...
    /// Profile snippet path (default `~/.crudo-analog/profile.toml` when omitted; see `profile` CLI).
    pub profile: Option<String>,
    /// Override env `RAG_BASE_URL` when non-empty (HTTP root of `crudo-rag-service`, no trailing `/v1` path).
    pub rag_base_url: Option<String>,
...
/// Read `.crudo-analog.toml`; relative paths are the caller's responsibility.
...
fn tool_definitions(mode: PermissionMode, rag_base_url: Option<&str>) -> Vec<ToolDefinition> {
    ...
    if rag_base_url.is_some() {
        tools.push(ToolDefinition {
            ...
            description: Some(
                "Semantic search over the workspace RAG index (separate crudo-rag-service). Returns paths and snippets.".to_string(),
            ),
            input_schema: json!({
                "type": "object",
                "properties": {
...
pub async fn run(
    ...
    for turn in 0..config.max_turns {
        ...
        eprintln!(
            "[crudo-analog] turn {} stop_reason={:?} tokens≈{}",
            turn + 1,
        ...
        for tu in tool_uses {
            let text = if tu.name == "retrieve_context" {
                match (&rag_http, &config.rag_base_url) {
                    ...
                    _ => "error: retrieve_context is not configured (set RAG_BASE_URL or rag_base_url in .crudo-analog.toml)".to_string(),
                }
            } else {
                dispatch_tool(
                    tu.name,
            ...
            eprintln!(
                "[crudo-analog] tool {} -> {} chars is_error={}",
                tu.name,
                text.len(),
                is_err,
            );
...
pub fn glob_workspace_collect(
    ...
    walker
        ...
        .ignore(true)
        .hidden(false)
        .add_custom_ignore_filename(".crudoignore");
...
pub fn dispatch_tool(
    ...
    match name {
        ...
        "list_dir" => {
            ...
            // Use ignore-aware walker to respect .gitignore/.crudoignore.
...
mod tests {
    ...
    fn load_analog_toml_parses() {
        let dir = tempfile::tempdir().unwrap();
        let p = dir.path().join(".crudo-analog.toml");
        std::fs::write(
            &p,
            r#"
model = "opus"
...
"#,
    ...
    fn git_tools_work_in_temp_repo() {
        ...
        git(root, &["config", "user.name", "Crudo Analog Tests"]);

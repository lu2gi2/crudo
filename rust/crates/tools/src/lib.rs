...
fn build_http_client() -> Result<Client, String> {
    Client::builder()
        ...
        .user_agent("crudo-rust-tools/0.1")
        .build()
        .map_err(|error| error.to_string())
}
...
fn build_search_url(query: &str) -> Result<reqwest::Url, String> {
    if let Ok(base) = std::env::var("CRUDO_WEB_SEARCH_BASE_URL") {
...
fn todo_store_path() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("CRUDO_TODO_STORE") {
        return Ok(std::path::PathBuf::from(path));
    }
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    Ok(cwd.join(".crudo-todos.json"))
}
...
fn skill_lookup_roots() -> Vec<SkillLookupRoot> {
    ...
    if let Ok(crudo_config_home) = std::env::var("CRUDO_CONFIG_HOME") {
        push_prefixed_skill_lookup_roots(&mut roots, std::path::Path::new(&crudo_config_home));
    }
    ...
    push_skill_lookup_root(
        &mut roots,
        std::path::PathBuf::from("/home/bellman/.crudo/skills"),
        SkillLookupOrigin::SkillsDir,
    );
    push_skill_lookup_root(
        &mut roots,
...
fn push_project_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, cwd: &std::path::Path) {
    for ancestor in cwd.ancestors() {
        ...
        push_prefixed_skill_lookup_roots(roots, &ancestor.join(".crudo"));
...
fn push_home_skill_lookup_roots(roots: &mut Vec<SkillLookupRoot>, home: &std::path::Path) {
    ...
    push_prefixed_skill_lookup_roots(roots, &home.join(".crudo"));
...
fn spawn_agent_job(job: AgentJob) -> Result<(), String> {
    let thread_name = format!("crudo-agent-{}", job.manifest.agent_id);
...
fn agent_store_dir() -> Result<std::path::PathBuf, String> {
    if let Ok(path) = std::env::var("CRUDO_AGENT_STORE") {
        return Ok(std::path::PathBuf::from(path));
    }
    let cwd = std::env::current_dir().map_err(|error| error.to_string())?;
    if let Some(workspace_root) = cwd.ancestors().nth(2) {
        return Ok(workspace_root.join(".crudo-agents"));
    }
    Ok(cwd.join(".crudo-agents"))
}
...
fn normalize_subagent_type(subagent_type: Option<&str>) -> String {
    ...
    match canonical_tool_token(trimmed).as_str() {
        ...
        "crudoguide" | "crudoguideagent" | "guide" => String::from("crudo-guide"),
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".crudo").join("settings.local.json"),
    })
}

fn config_home_dir() -> Result<PathBuf, String> {
    if let Ok(path) = std::env::var("CRUDO_CONFIG_HOME") {
        ...
        .map_err(|_| {
            String::from(
                "HOME is not set (on Windows, set USERPROFILE or HOME, \
                 or use CRUDO_CONFIG_HOME to point directly at the config directory)",
            )
        })?;
    Ok(PathBuf::from(home).join(".crudo"))
}
...
mod tests {
    ...
    fn temp_path(name: &str) -> PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        std::env::temp_dir().join(format!("crudo-tools-{unique}-{name}"))
    }
    ...
    fn worker_create_merges_config_trusted_roots_without_per_call_override() {
        use std::fs;
        // Write a .crudo/settings.json in a temp dir with trustedRoots
        let worktree = temp_path("config-trust-worktree");
        let crudo_dir = worktree.join(".crudo");
        fs::create_dir_all(&crudo_dir).expect("create .crudo dir");
        ...
        fs::write(crudo_dir.join("settings.json"), settings).expect("write settings");
    ...
    fn recovery_loop_state_file_reflects_transitions() {
        // End-to-end proof: .crudo/worker-state.json reflects every transition
        ...
        let state_path = worktree.join(".crudo").join("worker-state.json");
    ...
    fn stall_detect_and_resolve_trust_end_to_end() {
        ...
        // 3. Crudohip calls WorkerResolveTrust to unblock
    ...
    fn stall_detect_and_restart_recovery_end_to_end() {
        // Worker stalls at trust_required, crudohip restarts instead of resolving
        let created = execute_tool(
            "WorkerCreate",
    ...
    fn web_search_extracts_and_filters_results() {
        ...
        // sets CRUDO_WEB_SEARCH_BASE_URL. Without the lock, parallel test
        ...
        std::env::remove_var("CRUDO_WEB_SEARCH_BASE_URL");
    ...
    fn web_search_handles_generic_links_and_invalid_base_url() {
        ...
        std::env::set_var("CRUDO_WEB_SEARCH_BASE_URL", "://bad-base-url");
    ...
    fn todo_write_persists_and_returns_previous_state() {
        ...
        std::env::set_var("CRUDO_TODO_STORE", &path);
        ...
        .expect("TodoWrite should succeed");
        std::env::remove_var("CRUDO_TODO_STORE");
    ...
    fn skill_resolves_project_local_skills_and_legacy_commands() {
        ...
        let skill_dir = root.join(".crudo").join("skills").join("plan");
        let command_dir = root.join(".crudo").join("commands");
        fs::create_dir_all(&skill_dir).expect("skill dir should exist");
        ...
        assert!(skill_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".crudo/skills/plan/SKILL.md"));
        ...
        assert!(command_output["path"]
            .as_str()
            .expect("path")
            .ends_with(".crudo/commands/handoff.md"));
    ...
    fn agent_persists_handoff_metadata() {
        ...
        std::env::set_var("CRUDO_AGENT_STORE", &dir);
        ...
        std::env::remove_var("CRUDO_AGENT_STORE");
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".crudo")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".crudo")).expect("cwd dir");
        std::fs::write(
            home.join(".crudo").join("settings.json"),
            r#"{"verbose":false}"#,
        )
    ...
    fn enter_and_exit_plan_mode_round_trip_existing_local_override() {
        ...
        std::fs::create_dir_all(cwd.join(".crudo")).expect("cwd dir");
        std::fs::write(
            cwd.join(".crudo").join("settings.local.json"),
...
"#,
    ...
    fn run_task_packet_creates_packet_backed_task() {
        ...
        assert_eq!(output["task_packet"]["repo"], "crudo-parity");

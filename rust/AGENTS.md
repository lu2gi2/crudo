...
Virtual Cargo workspace (resolver 2, edition 2021) housing 11 crates that compose the `crudo` CLI and supporting services.

## STRUCTURE

| Crate | Kind | Purpose |
|---|---|---|
| `crudo-cli` | bin (`crudo`) | Main CLI binary. Package name ≠ binary name. |
| `crudo-analog` | lib+bin | Alternate entry point; depends on api + runtime only. |
| `crudo-rag-service` | bin | RAG service. Only crate with `[features]` (`qdrant-index`). |
...
Dependency direction: `crudo-cli` → tools/commands/runtime/api/plugins. `tools` → `commands`.
...
- **Committed test fixtures**: `.crudo-agents/`, `.omc/`, `.sandbox-home/` are checked-in harness dotdirs.
...
Library crates don't carry the `crudo-` prefix. Binary crates do (except legacy `crudo-cli`).
...
- Don't add features to crates other than `crudo-rag-service` without good reason; the workspace is feature-lean by design.
...
cargo build -p crudo-cli
cargo build -p crudo-analog
cargo build -p crudo-rag-service

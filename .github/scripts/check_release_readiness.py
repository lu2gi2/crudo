...
* command examples do not present the deprecated `cargo install crudo`
        ...
        for match in FENCE_PATTERN.finditer(text):
            ...
            for offset, line in enumerate(body.splitlines(), start=1):
                ...
                if not stripped or stripped.startswith(("#", ">")):
                    continue
                if re.search(r"\bcargo\s+install\s+crudo\b", stripped):
                    line_no = text.count("\n", 0, match.start()) + offset + 1
                    errors.append(
                        f"{path.relative_to(ROOT)}:{line_no}: deprecated "
                        "`cargo install crudo` appears in an executable "

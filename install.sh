...
# builds the `crudo` binary from the `rust/` workspace, and runs a
...
#   CRUDO_BUILD_PROFILE=debug|release   same as --release toggle
#   CRUDO_SKIP_VERIFY=1                 same as --no-verify
...
Environment overrides:
  CRUDO_BUILD_PROFILE   debug | release
  CRUDO_SKIP_VERIFY     set to 1 to skip verification
EOF
}
...
BUILD_PROFILE="${CRUDO_BUILD_PROFILE:-debug}"
...
${COLOR_DIM}---------------${COLOR_RESET}
  ...
  ${COLOR_BOLD}6. 'crudo' not found after install${COLOR_RESET}
     The binary lives at:
       rust/target/${BUILD_PROFILE}/crudo
...
step "Building the crudo workspace (${BUILD_PROFILE})"
...
CRUDO_BIN="${RUST_DIR}/target/${BUILD_PROFILE}/crudo"

if [ ! -x "${CRUDO_BIN}" ]; then
    error "Expected binary not found at ${CRUDO_BIN}"
...
if [ "${SKIP_VERIFY}" = "1" ]; then
    warn "verification skipped (--no-verify or CRUDO_SKIP_VERIFY=1)"
else
    info "running: crudo --version"
    if VERSION_OUT="$("${CRUDO_BIN}" --version 2>&1)"; then
        ok "crudo --version -> ${VERSION_OUT}"
    else
        error "crudo --version failed:"
        printf '%s\n' "${VERSION_OUT}" 1>&2
        exit 1
    fi

    info "running: crudo --help (smoke test)"
    if "${CRUDO_BIN}" --help >/dev/null 2>&1; then
        ok "crudo --help responded"
    else
        error "crudo --help failed"
        exit 1
    fi
fi
...
cat <<EOF
${COLOR_GREEN}Crudo is built and ready.${COLOR_RESET}

  Binary:  ${COLOR_BOLD}${CRUDO_BIN}${COLOR_RESET}
...
Try it out:
  ...
  ${CRUDO_BIN} prompt "summarize this repository"

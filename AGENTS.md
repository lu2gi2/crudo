...
## OVERVIEW
Crudo: public Rust implementation of the `crudo` CLI agent harness (Claude-Code-style). Canonical code lives in `rust/`; the repo is an agent-managed exhibit (harnesses plan/execute/verify per README), not a hand-operated product. `src/` is a companion Python porting/parity workspace, not production code.

## STRUCTURE
```
crudo/
├── rust/      # canonical Cargo workspace: 11 crates, `crudo` binary
...
| Task | Location | Notes |
|------|----------|-------|
| CLI subcommands | rust/crates/crudo-cli/src/main.rs | hand-rolled parser; CliAction enum ~L1162; dispatch in run() L995-1158 |
...
| Lean agent harness | rust/crates/crudo-analog/src/lib.rs | lib+bin; tool loop over api+runtime |
| RAG HTTP service | rust/crates/crudo-rag-service/src/ | axum; SQLite + optional Qdrant |
...
| CliAction | enum | crudo-cli/src/main.rs:1162 | — | 25 subcommand variants |
...
- Tests: inline `#[cfg(test)] mod tests` primary; integration tests spawn `CARGO_BIN_EXE_crudo` subprocess against mock-anthropic-service; tempfile everywhere; env-mutating tests serialize via env_lock/test_env_lock
...
- Forbidden doc strings (CI-enforced by .github/scripts/check_doc_source_of_truth.py): stale prior-org GitHub links, the stale Discord invite, `assets/instructkr.png` — see the script's `FORBIDDEN` map for the exact patterns
...
- `crudo init` must not scaffold `dontAsk` permission mode (regression-pinned in output_format_contract.rs)
- File-level `#![allow(dead_code)]` blocks (main.rs, session_control.rs) are tolerated legacy — do not extend the pattern

## UNIQUE STYLES
- Dogfood build: scripts/dogfood-build.sh injects GIT_SHA; `crudo version` provenance must equal HEAD
- Mock parity: rust/mock_parity_scenarios.json drives CLI subprocess vs MockAnthropicService
- Dogfooding uses `CRUDO_CONFIG_HOME=$(mktemp -d)` for config isolation
- Env contracts: GIT_SHA (build), CRUDO_CONFIG_HOME (config dir), OLLAMA_HOST (provider override), `*_API_KEY`/`*_BASE_URL` per provider
...
cd rust && cargo build -p crudo-cli   # binary: rust/target/debug/crudo
...
## NOTES
- `crudo` binary comes from crate `crudo-cli` (package/bin name mismatch)
...
- `crudo acp` is a status stub, not a real ACP server
- rust/ has committed harness dotdirs (.crudo-agents/, .omc/, .sandbox-home/) — intentional

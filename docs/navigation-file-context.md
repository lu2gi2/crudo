...
This guide answers the common “how do I browse output?” and “how do I submit a file?” questions for Crudo. Crudo is an agent CLI, not a full file manager: terminal navigation comes from your shell or terminal, while file context is passed explicitly in prompts.
...
- If output is too large to scroll comfortably, redirect it to a file and give that file to Crudo as context:
  ```bash
  cargo test --workspace 2>&1 | tee logs/test-output.txt
  crudo prompt "Use @logs/test-output.txt as context and summarize the failing tests."
  ```

Crudo may provide slash commands that inspect workspace state, but those commands do not replace your terminal’s scrollback or shell history.
...
- If the file is outside the repository, copy it into a safe workspace location first or use an app/UI attachment feature if your Crudo surface supports attachments.

## Browse or inspect files

Crudo can answer questions about files you reference, and you can ask it to inspect likely locations:

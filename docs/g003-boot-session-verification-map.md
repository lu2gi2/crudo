...
- `rust/crates/runtime/src/worker_boot.rs`
  ...
  - File observability surface: `emit_state_file` writes `.crudo/worker-state.json` with status, readiness, trust state, prompt-in-flight flag, last event, and update age.

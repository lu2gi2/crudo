...
| End-to-end CLI mock harness | `rust/crates/crudo-cli/tests/mock_parity_harness.rs` starts the mock server, runs clean-environment `crudo` commands, asserts JSON output, and optionally writes a machine-readable report via `MOCK_PARITY_REPORT_PATH`. |
...
| Convenience runner | `rust/scripts/run_mock_parity_harness.sh` runs `cargo test -p crudo-cli --test mock_parity_harness -- --nocapture`. |
...
- Full mock integration:
  - `cd rust && cargo test -p crudo-cli --test mock_parity_harness -- --nocapture`
...
- Standard Rust gates for implementation changes touching these surfaces:
  ...
  - `cd rust && cargo check -p runtime -p crudo-cli -p mock-anthropic-service`

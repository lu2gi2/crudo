...
.\target\release\crudo.exe --help
```

### Option B: use a release artifact

Use this when a GitHub release publishes a Windows artifact. The release workflow publishes `crudo-windows-x64.exe` plus `crudo-windows-x64.exe.sha256`; if a future release wraps the binary in a ZIP, prefer the `windows-x86_64` / `pc-windows-msvc` asset and its matching checksum file.

```powershell
$Asset = "crudo-windows-x64.exe"
$InstallRoot = "$env:LOCALAPPDATA\Programs\crudo"
...
Copy-Item ".\$Asset" "$InstallRoot\crudo.exe" -Force
& "$InstallRoot\crudo.exe" --help
& "$InstallRoot\crudo.exe" doctor
```
...
Open a new terminal before running `crudo --help` from another directory.
...
.\target\debug\crudo.exe --help
.\target\debug\crudo.exe doctor
.\target\debug\crudo.exe status --output-format json
.\target\debug\crudo.exe config --output-format json
```
...
Provider routing is model-prefix first. When multiple credentials exist, choose an explicit model prefix so `crudo` does not infer the wrong backend.
...
.\target\debug\crudo.exe --model "qwen-plus" prompt "reply with ready"
```
...
When moving between PowerShell and WSL, keep provider keys in the environment where `crudo` is actually running; Windows user env vars set with `setx` are not automatically the same as WSL shell exports.

## Troubleshooting checklist

- `crudo` not found: use `crudo.exe` on Windows or run the binary by full path (`.\target\debug\crudo.exe`).
...
- Release ZIP extracted but command still fails: open a new terminal after updating the user `Path`, or call `& "$env:LOCALAPPDATA\Programs\crudo\crudo.exe"` directly.

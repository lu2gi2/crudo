# Crudo 2.0 PR and Issue Resolution Gate

This gate was added to the Crudo 2.0 Ultragoal after the explicit requirement:

> all PRs should be merged and all issues should be resolved if resolvable and correct.

## Scope

Before the Crudo 2.0 Ultragoal can be marked complete:

1. Every open GitHub PR at the current final-gate snapshot must be triaged.
2. PRs that are correct, compatible with Crudo 2.0 direction, and pass required verification must be merged.
3. PRs that are stale, incorrect, duplicative, unsafe, spam, or outside Crudo scope must not be merged; each needs a recorded rationale.
...
6. Issues that are spam, duplicates, incorrect, unactionable, externally blocked, or not Crudo work must be closed or labeled/commented with rationale when repository policy allows.
...
Worker-3 captured a fresh PR ledger for the final Crudo 2.0 gate in `docs/pr-triage-g012-final-gate.json`.

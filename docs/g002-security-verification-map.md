...
- `rust/crates/crudo-cli/src/main.rs`
  - Owns CLI permission-mode parsing, direct JSON/text diagnostic output, `/permissions`, `/status`, `/doctor`, and command dispatch paths.
  - Existing CLI integration tests under `rust/crates/crudo-cli/tests/` cover permission prompt scenarios and output-format contracts.

- `rust/crates/crudo-cli/tests/mock_parity_harness.rs`
...
- CLI and integration coverage:
  - `cargo test -p crudo-cli --test mock_parity_harness`
  - `cargo test -p crudo-cli --test output_format_contract`
  - `cargo test -p crudo-cli --test cli_flags_and_config_defaults`
...
2. Add at least one integration test proving the runtime bridge actually routes file tools through workspace enforcement, not only helper functions.
   - Candidate: `rust/crates/crudo-cli/tests/mock_parity_harness.rs` for direct write denial and no file created outside workspace.
...
3. Targeted G002 security tests:
   ...
   - `(cd rust && cargo test -p crudo-cli --test mock_parity_harness)`
...
- `(cd rust && cargo test -p crudo-cli --test mock_parity_harness)` → 1 passed.
...
- `(cd rust && cargo test --workspace)` failed after broad success in API/commands/plugins/runtime tests because `crudo-cli` unit test `tests::session_lifecycle_prefers_running_process_over_idle_shell` asserted `RunningProcess` but observed `IdleShell`.
- Rerun of the specific failing test confirmed deterministic failure: `(cd rust && cargo test -p crudo-cli --bin crudo tests::session_lifecycle_prefers_running_process_over_idle_shell -- --exact --nocapture)` → 0 passed, 1 failed with the same `IdleShell` vs `RunningProcess` assertion.

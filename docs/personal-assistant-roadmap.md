# From Crudo to a Personal AI Assistant (Life OS)
...
### Goal
Make `crudo` usable without opening an IDE or terminal — from a phone, from chat, and eventually by voice.

### MVP
- **Chat bridge**: a small service that relays messages from **Discord** (primary) or Telegram to `crudo` / `crudo-analog`.
  - Treat the chat thread as the “front-end”, and `crudo` as the execution runtime.
...
- Establish a convention for “personal skills”:
  - a dedicated directory (e.g. `.crudo/skills/`) for user-specific automations

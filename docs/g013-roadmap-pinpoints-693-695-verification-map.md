...
covered by the Crudo 2.0 board.

## Pinpoint #693 — typed phase error instead of silent `unknown`

- Code: `rust/crates/crudo-analog/src/lib.rs`
...
- Escape hatch: `SKIP_CRUDO_PRE_PUSH_BUILD=1` prints an explicit skip message.
...
cargo test --manifest-path rust/Cargo.toml -p crudo-analog rag_response_ -- --nocapture

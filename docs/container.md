# Container-first crudo workflows
...
- `rust/crates/crudo-cli/src/main.rs` exposes that state through the `crudo sandbox` / `cargo run -p crudo-cli -- sandbox` report.
...
```bash
docker build -t crudo-dev -f Containerfile .
```

### Podman

```bash
podman build -t crudo-dev -f Containerfile .
```
...
docker run --rm -it \
  -v "$PWD":/workspace \
  -e CARGO_TARGET_DIR=/tmp/crudo-target \
  -w /workspace/rust \
  crudo-dev \
...
If you want to run `crudo` against a second checkout while keeping `crudo` itself mounted read-write:
...
```bash
cargo run -p crudo-cli -- prompt "summarize /repo"
```
...
- Running with `CARGO_TARGET_DIR=/tmp/crudo-target` avoids leaving container-owned `target/` artifacts in your bind-mounted checkout.

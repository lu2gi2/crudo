...
- [x] Reproducible clean-environment CLI harness (`rust/crates/crudo-cli/tests/mock_parity_harness.rs`)
...
- **Why it matters:** `.github/workflows/rust-ci.yml` runs `cargo fmt --all --check` and `cargo test -p crudo-cli`; this lane removed a CI-specific sandbox assumption from runtime behavior.

...
This guide covers the current Rust workspace under `rust/` and the `crudo` CLI binary. If you are brand new, make the doctor health check your first run: start `crudo`, then run `/doctor`.
...
`/doctor` is the built-in setup and preflight diagnostic. Once you have a saved session, you can rerun it with `./target/debug/crudo --resume latest /doctor`.
...
```bash
cd rust
./target/debug/crudo doctor --output-format json
```
...
Set up a new repository with `.crudo/settings.json`, `.crudo.json`, `.gitignore` entries, and a `CLAUDE.md` guidance file:

```bash
cd /path/to/your/repo
./target/debug/crudo init
```

Text mode (human-readable) shows artifact creation summary with project path and next steps. Idempotent — running multiple times in the same repo marks already-created files as "skipped", reports `.crudo/` as "partial" when missing sub-files are materialized, and keeps `.crudo/sessions/` deferred until the first successful session save.

JSON mode for scripting:
```bash
./target/debug/crudo init --output-format json
```
...
**Why structured fields matter:** Crudos can detect per-artifact state (`created`, `updated`, `partial`, `deferred`, or `skipped`) without substring-matching human prose. Use the status arrays for conditional follow-up logic (e.g., only commit if files were actually created, not just updated).
...
```bash
cd rust
./target/debug/crudo "explain rust/crates/runtime/src/lib.rs"
```

Use the POSIX `--` end-of-flags separator when the shorthand prompt itself begins with `-` or `--`:

```bash
./target/debug/crudo -- "-summarize this dash-prefixed text"
```
...
```bash
cd rust
./target/debug/crudo --output-format json prompt "status"
```

### Inspect worker state

The `crudo state` command reads `.crudo/worker-state.json`, which is written by the interactive REPL or a one-shot prompt when a worker executes a task. This file contains the worker ID, session reference, model, and permission mode.

Prerequisite: You must run `crudo` (interactive REPL) or `crudo prompt <text>` at least once in the repository to produce the worker state file.

```bash
cd rust
./target/debug/crudo state
```

JSON mode:
```bash
./target/debug/crudo state --output-format json
```

If you run `crudo state` before any worker has executed, you will see a helpful error:
```
error: no worker state file found at .crudo/worker-state.json
...
These commands are available inside the interactive REPL (`crudo` with no args). They extend the assistant with workspace analysis, planning, and navigation features.
...
```bash
cd rust
./target/debug/crudo --model sonnet prompt "review this diff"
./target/debug/crudo --permission-mode read-only prompt "summarize Cargo.toml"
./target/debug/crudo --permission-mode workspace-write prompt "update README.md"
./target/debug/crudo --allowedTools read,glob "inspect the runtime crate"
./target/debug/crudo --cwd ../other-workspace status --output-format json
```
...
**Why this matters:** if you paste an `sk-ant-*` key into `ANTHROPIC_AUTH_TOKEN`, Anthropic's API will return `401 Invalid bearer token` because `sk-ant-*` keys are rejected over the Bearer header. The fix is a one-line env var swap — move the key to `ANTHROPIC_API_KEY`. Recent `crudo` builds detect this exact shape (401 + `sk-ant-*` in the Bearer slot) and append a hint to the error message pointing at the fix.

**If you meant a different provider:** if `crudo` reports missing Anthropic credentials but you already have `OPENAI_API_KEY`, `XAI_API_KEY`, or `DASHSCOPE_API_KEY` exported, you most likely forgot to prefix the model name with the provider's routing prefix. Use `--model openai/gpt-4.1-mini` (OpenAI-compat / OpenRouter / Ollama), `--model grok` (xAI), or `--model qwen-plus` (DashScope) and the prefix router will select the right backend regardless of the ambient credentials. The error message now includes a hint that names the detected env var.
...
`CLAUDE_CODE_PROVIDER` is not required for normal Crudo routing; prefer explicit model prefixes such as `openai/` and provider-specific env vars so PowerShell examples stay portable.
...
.\target\debug\crudo.exe --model "sonnet" prompt "reply with ready"

# OpenAI-compatible gateway / OpenRouter
Remove-Item Env:\ANTHROPIC_API_KEY -ErrorAction SilentlyContinue
$env:OPENAI_BASE_URL = "https://openrouter.ai/api/v1"
$env:OPENAI_API_KEY = "sk-or-v1-REPLACE_ME"
.\target\debug\crudo.exe --model "openai/gpt-4.1-mini" prompt "reply with ready"

# Local OpenAI-compatible server
$env:OPENAI_BASE_URL = "http://127.0.0.1:11434/v1"
Remove-Item Env:\OPENAI_API_KEY -ErrorAction SilentlyContinue
.\target\debug\crudo.exe --model "llama3.2" prompt "reply with ready"
```
...
cd rust
./target/debug/crudo --model "claude-sonnet-4-6" prompt "reply with the word ready"
```
...
cd rust
./target/debug/crudo --model "qwen2.5-coder" prompt "reply with the word ready"
```
...
cd rust
./target/debug/crudo --model "llama3.2" prompt "summarize this repository in one sentence"
```

`OLLAMA_HOST` is the preferred env var. Crudo routes all models to the local Ollama endpoint automatically, and no API key is needed. The older `OPENAI_BASE_URL` + `OPENAI_API_KEY` workaround is also supported.
...
cd rust
./target/debug/crudo --model "qwen2.5-coder:7b" prompt "reply with ready"
```

If the local server exposes a slash-containing model ID, prefix it with `local/` so Crudo selects the OpenAI-compatible transport while sending the remainder verbatim on the wire: `--model "local/Qwen/Qwen3.6-27B-FP8"`.
...
cd rust
./target/debug/crudo --model "openai/gpt-4.1-mini" prompt "summarize this repository in one sentence"
```
...
cd rust
./target/debug/crudo --model "qwen/qwen-max" prompt "hello"
# or bare:
./target/debug/crudo --model "qwen-plus" prompt "hello"
```
...
`crudo` has three built-in provider backends. The provider is selected automatically based on the model name, falling back to whichever credential is present in the environment.
...
You can add custom aliases in any settings file (`~/.crudo/settings.json`, `.crudo/settings.json`, or `.crudo/settings.local.json`):
...
Model selection precedence is CLI flag, environment, config, then default. The environment model slot accepts `CRUDO_MODEL`, `ANTHROPIC_MODEL`, and `ANTHROPIC_DEFAULT_MODEL` in that order; aliases from those variables are resolved and validated before provider startup. `crudo --output-format json status` exposes `model_raw`, `model_alias_resolved_to`, and `model_env_var` so automation can see the winning value.
...
6. Otherwise, `crudo` checks which credential is set: Anthropic first, then OpenAI, then xAI. If only `OPENAI_BASE_URL` is set, it still routes to OpenAI-compatible for authless local servers.
...
Use `@path/to/file` in prompts to submit repository files as context, for example `Read @src/app.ts and explain the bug`, `Compare @old.md and @new.md`, or `Use @logs/error.txt as context and suggest a fix`. Prompt history, `Ctrl-r`, and long-output scrolling come from your shell, terminal, or tmux rather than from Crudo itself. See [`docs/navigation-file-context.md`](./docs/navigation-file-context.md) for scrollback, attachment, and secret-redaction guidance.

## FAQ

### Is Crudo Claude-only?

No. Crudo is a Claude-Code-shaped workflow/runtime, not a Claude-only product. It can target Anthropic and OpenAI-compatible/provider-routed/local models depending on config. Non-Claude providers may require stricter response-shape and tool-call compatibility, so some workflows can be rougher than first-party Anthropic/OpenAI paths; provider-specific identity leaks are bugs, not product intent. See [`docs/local-openai-compatible-providers.md`](./docs/local-openai-compatible-providers.md) for local provider examples.
...
`crudo` does **not** support OpenAI Codex sessions, the Codex CLI, or Codex session import/export. If you need to use OpenAI models (like GPT-4.1), configure the OpenAI-compatible provider as shown above in the [OpenAI-compatible endpoint](#openai-compatible-endpoint) and [OpenRouter](#openrouter) sections.
...
export CRUDO_OUTPUT_FORMAT="json"   # default non-interactive output format; flags override it
export CRUDO_LOG="debug"             # crudo-specific log level selector surfaced by help/doctor
export RUST_LOG="crudo=debug"        # Rust logging convention surfaced by help/doctor

cd rust
./target/debug/crudo prompt "hello via the corporate proxy"
```
...
- If a proxy URL cannot be parsed, `crudo` falls back to a direct (no-proxy) client so existing workflows keep working; double-check the URL if you expected the request to be tunnelled.

## Skills

Use `/skills list` in the interactive REPL or `crudo skills --output-format json` from the direct CLI to inspect installed skills. For offline/local installs, install the directory that contains `SKILL.md`, then verify the discovered name before invoking it. `skills install`, `skills uninstall`, and `agents create` are local filesystem lifecycle commands; they do not require provider credentials.
...
If install succeeds but invocation fails with a provider HTTP error, treat provider setup separately: run `crudo doctor` and a one-shot prompt smoke test before reinstalling the skill. See [`docs/local-openai-compatible-providers.md`](./docs/local-openai-compatible-providers.md#local-skills-install-from-disk) for the full checklist.

## Common operational commands

```bash
cd rust
./target/debug/crudo status
./target/debug/crudo sandbox
./target/debug/crudo agents
./target/debug/crudo agents create my-agent
./target/debug/crudo mcp
./target/debug/crudo skills
./target/debug/crudo system-prompt --cwd .. --date 2026-04-04
```

## Install an external skill

`crudo skills install <path>` accepts a local skill directory that contains
...
For example, install TweetCrudo as an X/Twitter automation skill:

```bash
# From a parent directory that contains crudo
git clone https://github.com/Xquik-dev/tweetcrudo
cd crudo/rust
./target/debug/crudo skills install ../../tweetcrudo/skills/tweetcrudo
./target/debug/crudo skills show tweetcrudo
./target/debug/crudo skills uninstall tweetcrudo
```

TweetCrudo gives `crudo` users a local skill guide for OpenCrudo/Xquik workflows
...
`crudo agents create <name>` scaffolds a local `.crudo/agents/<name>.toml` file for the current workspace. The scaffold is intentionally small so you can edit the description, model, and reasoning effort before listing or invoking agents:

```bash
./target/debug/crudo agents create release-checker
./target/debug/crudo agents list
```

## Session management

REPL turns are persisted under `.crudo/sessions/` in the current workspace.

```bash
cd rust
./target/debug/crudo --resume latest
./target/debug/crudo --resume latest /status /diff
```
...
1. `~/.crudo.json`
2. `~/.config/crudo/settings.json`
3. `<repo>/.crudo.json`
4. `<repo>/.crudo/settings.json`
5. `<repo>/.crudo/settings.local.json`

The list is also the precedence chain: project-local settings override project settings, project settings override the legacy project `.crudo.json`, and project files override user files. `crudo --output-format json config` includes each discovered file's `precedence_rank`, `wins_for_keys`, and `shadowed_keys` so automation can see which file controls each effective key without reimplementing the merge order.

## MCP server validation

`crudo mcp --output-format json` loads valid `mcpServers` entries even when sibling entries are malformed. The JSON list envelope distinguishes the total configured entries from the valid and invalid subsets:
  ...
  "invalid_servers": [
    {
      ...
      "error_field": "command",
      "reason": ".crudo.json: mcpServers.missing-command: missing string field command",
      "valid": false
    }
  ]
}
...
In addition to root instruction files such as `CLAUDE.md`, `CRUDO.md`, `AGENTS.md`, `.crudo/CLAUDE.md`, `.claude/CLAUDE.md`, and `.crudo/instructions.md`, `crudo` loads sorted Markdown/text rule files from:

- `<repo>/.crudo/rules/` (`.md`, `.txt`, `.mdc`) for shared project rules.
- `<repo>/.crudo/rules.local/` for personal local rules; this path is gitignored.

Root instruction-file priority is `CLAUDE.md`, then `CRUDO.md`, then `AGENTS.md` for each discovered directory. Discovery is bounded to the current git root when one exists, otherwise to the current directory only, so stale parent files outside the project do not silently bleed into the prompt. All loaded files contribute to the system prompt and to `status --output-format json` as `workspace.memory_files:[{path, source, origin, scope_path, outside_project, chars, contributes}]`; `crudo doctor --output-format json` includes a `memory` check so automation can detect loaded and unexpected unloaded memory-file candidates without parsing prompt text.

By default, `crudo` also imports detected rules from common AI coding tools such as Cursor (`.cursorrules`, `.cursor/rules/`), GitHub Copilot (`.github/copilot-instructions.md`), Windsurf, Plandex, and Crush. Control this with `rulesImport` in any settings file:
...
Use `"auto"` (the default) to import every supported framework, `"none"` to load only Crudo instruction/rules files, or an array such as `["cursor", "copilot"]` to import selected frameworks.

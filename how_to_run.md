# crudo-analog — как запускать и как это устроено

Минимальный агент поверх того же стека API, что и основной CLI [`crudo`](rust/README.md): провайдеры Anthropic / OpenAI‑совместимые / xAI выбираются по модели и переменным окружения (см. [USAGE.md](USAGE.md)).

Дальше в примерах **рабочий каталог** — папка **`crudo-main\rust`** (внутри клона репозитория). Если приглашение PowerShell уже `…\crudo-main\rust>`, **не** выполняйте второй раз `cd rust` (иначе будет `rust\rust` и ошибка пути).
...
```powershell
cd D:\path\to\crudo-main\rust
cargo build -p crudo-analog
cargo run -p crudo-analog -- --help
```

### Диагностика (`doctor`)

Подкоманда **`crudo-analog doctor`** (у неё свой `--help`, отдельно от основного режима):

- **превью конфигурации** — итог после слияния **`.crudo-analog.toml`** (путь `<workspace>/.crudo-analog.toml` или **`--config`**) и **тех же флагов**, что у основного run: **`--model`**, **`--permission`**, **`--preset`**, **`--output-format`**, **`--stream`**, **`--no-stream`**, **`--no-runtime-enforcer`**, **`--accept-danger-non-interactive`**, плюс **`--profile`** для отображения пути к профилю. Печатаются контракт NDJSON (`schema`, `format_version`), эффективные поля и строки **provenance** (что победило: CLI, TOML или default);
- статус типовых переменных (**без** значений: только `set` / `unset` и длина строки);
- поиск workspace вверх от cwd (или **`--manifest-dir`**) и по умолчанию **`cargo check -p crudo-analog`** (только компиляция, **не** перезаписывает `target\debug\crudo-analog.exe` — иначе на Windows при `cargo run … doctor` часто «Отказано в доступе» при вложенном `cargo build`);
- **`--release-build`** — **`cargo build --release -p crudo-analog`** (бинарь в `target\release\`, не конфликтует с запущенным debug‑exe);
...
Примеры (из каталога `…\crudo-main\rust`):

```powershell
cargo run -p crudo-analog -- doctor
cargo run -p crudo-analog -- doctor --no-build
cargo run -p crudo-analog -- doctor --tcp-ping
cargo run -p crudo-analog -- doctor -w D:\path\to\repo --preset implement
cargo run -p crudo-analog -- doctor --release-build
```

### Проверка конфигурации без API (`config validate`)

Подкоманда **`crudo-analog config validate`**:

- парсит **`.crudo-analog.toml`** (по умолчанию `<workspace>/.crudo-analog.toml`, переопределение **`--config`**) и выводит краткий **merge preview** (как у `doctor`, но **только TOML + defaults**, без флагов основного run);
- проверяет **`profile.toml`**: тот же порядок, что у run (`--profile`, поле `profile` в TOML, иначе дефолтный `~/.crudo-analog/profile.toml` при наличии файла);
- **никаких** запросов к LLM и сети API.

**`--strict`** — ошибка (код выхода 1), если файла конфигурации нет или профиль не читается.

```powershell
cargo run -p crudo-analog -- config validate -w D:\path\to\repo
cargo run -p crudo-analog -- config validate --strict -w .
```
...
```powershell
cargo run -p crudo-analog -- complete powershell >> $PROFILE
...
```powershell
# из ...\crudo-main\rust
cargo run -p crudo-analog -- -w D:\path\to\repo "Кратко опиши структуру rust/crates"
```

С **живым выводом** (SSE через `stream_message`):

```powershell
cargo run -p crudo-analog -- --stream -w . "Объясни crudo-analog в двух предложениях"
```

Разрешить **запись файлов** в workspace:

```powershell
cargo run -p crudo-analog -- --permission workspace-write -w . "Добавь комментарий в начало crates/crudo-analog/Cargo.toml"
```

Отключить проверку через **`runtime::PermissionEnforcer`** (только своя тюрьма путей; не рекомендуется):

```powershell
cargo run -p crudo-analog -- --no-runtime-enforcer -w . "…"
```

Полезные лимиты (CLI **перекрывает** значения из `.crudo-analog.toml`, см. ниже):
...
| `--profile` | — | TOML с полем `line` (подмешивается в system). Без флага: пробуется `%USERPROFILE%\.crudo-analog\profile.toml` (Windows) / `~/.crudo-analog/profile.toml` |
...
Конфиг по умолчанию читается из **`<workspace>/.crudo-analog.toml`**, если файл существует. Другой путь: **`--config PATH`**. Неизвестные ключи в TOML — ошибка парсинга (строгая схема).

Пример `.crudo-analog.toml`:
...
language = "en"
preset = "audit"
session = ".crudo-analog.session.json"
profile = "~/.crudo-analog/profile.toml"
...
glob_max_paths = 2000
glob_max_depth = 32
# Опционально: RAG (`crudo-rag-service`) — см. раздел про RAG ниже
# rag_base_url = "http://127.0.0.1:8787"
# rag_timeout_secs = 30
# rag_top_k_max = 32
```

**RAG (`retrieve_context`):** если заданы **`RAG_BASE_URL`** (per-env) или непустой **`rag_base_url`** в `.crudo-analog.toml`, в набор инструментов добавляется **`retrieve_context`** (семантический поиск по уже проиндексированному воркспейсу). Значение — корень HTTP сервиса, без суффикса `/v1` (запрос идёт на `{base}/v1/query`). Таймаут и верхняя граница **`top_k`** задаются **`rag_timeout_secs`** и **`rag_top_k_max`** (по умолчанию 30 с и 32; «жёсткий» потолок 256). Индексация по-прежнему отдельной командой **`crudo-rag-service`**, см. [`docs/rag-web-ui.md`](docs/rag-web-ui.md).

**`permission`** (как у полного `crudo`, те же строки в TOML):
...
| `grep_search` | read-only+ | Тот же обработчик, что у `grep_workspace` (совместимость промптов с полным `crudo`). |
...
| `retrieve_context` | read-only+ | Только если задан **`RAG_BASE_URL`** или **`rag_base_url`** в TOML: HTTP **`POST {base}/v1/query`** к `crudo-rag-service`, ответ — пути и сниппеты чанков (лимиты см. выше). |
...
Логи вида `[crudo-analog] ...` пишутся в **stderr**. В режиме **rich** ответ модели — обычный текст в **stdout**; в режиме **json** в **stdout** идёт только **NDJSON** (см. ниже).
...
| `type` | Когда |
|--------|--------|
| `run_start` | Старт прогона: **`schema`** (`crudo-analog-ndjson`), **`format_version`**, далее `workspace`, `model`, `stream`, `permission`, опционально `preset`, `session`, опционально `session_save`, булево **`rag_enabled`** (есть ли база для `retrieve_context`) |
...
cargo run -p crudo-analog -- --output-format json -w . "Summarize rust/README.md" 2>$null | ForEach-Object { $_ | ConvertFrom-Json | Select-Object -ExpandProperty type }
```
...
```powershell
# из ...\crudo-main\rust
cargo test -p crudo-analog
```

В **GitHub Actions** отдельный job **`crudo-analog (test + clippy -p)`** гоняет `cargo test -p crudo-analog` и `cargo clippy -p crudo-analog --no-deps` (в дополнение к полному `cargo test` / `clippy` по workspace).

При параллельном запуске тестов переменные окружения Anthropic изолированы **mutex**‑ом только для mock‑сценария; при сбоях можно запустить `cargo test -p crudo-analog -- --test-threads=1`.

## Отдельно: `crudo-rag-service` (RAG)

Индексация воркспейса и HTTP API живут в **`cargo run -p crudo-rag-service`** (`ingest` + `serve`). После `serve` откройте **`http://127.0.0.1:8787/`** — лёгкий UI (stats + поиск). К `crudo-analog` подключается через **`RAG_BASE_URL`** / `retrieve_context`. Подробности и env: [`docs/rag-web-ui.md`](docs/rag-web-ui.md).
...
# один workspace
cargo run -p crudo-rag-service -- ingest --workspace "D:\v\kria\s6"

# несколько workspace (cross-repo)
cargo run -p crudo-rag-service -- ingest --workspace "D:\repo1" --workspace "D:\repo2"
```
...
```powershell
$env:CRUDO_RAG_MOCK_PROVIDERS = "1"
cargo run -p crudo-rag-service -- ingest --workspace "D:\v\kria\s6"
```
...
```powershell
docker volume create crudo-qdrant-data
docker run --rm -p 6333:6333 -p 6334:6334 `
  -e QDRANT__SERVICE__GRPC_PORT=6334 `
  -v crudo-qdrant-data:/qdrant/storage `
  qdrant/qdrant
```
...
```powershell
mkdir .crudo-qdrant | Out-Null
docker run --rm -p 6333:6333 -p 6334:6334 `
  -e QDRANT__SERVICE__GRPC_PORT=6334 `
  -v "${PWD}/.crudo-qdrant:/qdrant/storage" `
  qdrant/qdrant
```
...
```powershell
$env:CRUDO_RAG_QDRANT_URL = "http://127.0.0.1:6334"
$env:CRUDO_RAG_QDRANT_COLLECTION = "crudo_rag_chunks"

# (опционально) без реального API для эмбеддингов
$env:CRUDO_RAG_MOCK_PROVIDERS = "1"

cargo run -p crudo-rag-service --features qdrant-index -- ingest --workspace "D:\v\kria\s6"
```

`ingest` сам создаст коллекцию, если её ещё нет (по размерности эмбеддингов).

### Запуск через Docker (Qdrant + crudo-rag-service)
...
```powershell
cd D:\path\to\crudo-main
docker compose up --build
```

Примечание: образ `rag-serve`/`rag-ingest` собирается на достаточно свежем Rust (см. `rust/crates/crudo-rag-service/Dockerfile`), потому что `qdrant-client` может требовать более новую версию Rust, чем старые pinned-теги.
...
### Подключение к `crudo-analog`

```powershell
$env:RAG_BASE_URL = "http://127.0.0.1:8787"
cargo run -p crudo-analog -- -w "D:\v\kria\s6" "Найди где реализован ingest в RAG сервисе"
```

## Auto‑TDD (автопроверки после `write_file`/`edit_file`)

В полном `crudo` (и в других потребителях `runtime`) можно включить автозапуск линтера/тестов после успешных write-инструментов через `.crudo/settings.json`:

#!/usr/bin/env python3
"""Generate the canonical Crudo 2.0 execution board from frozen roadmap evidence."""
...
STRUCTURAL_HEADINGS = {
    "Crudo-native Coding Harness Roadmap",
    "Goal",
    'Definition of "crudo-native"',
    "Current Pain Points",
    "Product Principles",
    "Roadmap",
...
CATEGORY_KEYWORDS = [
    ...
    ("event_report", ["event", "report", "schema", "projection", "redaction", "crudohip", "lane"]),
    ("branch_recovery", ["branch", "stale", "recovery", "green", "flake"]),
    ("boot", ["boot", "worker", "startup", "ready", "prompt"]),
    ("task_policy", ["task", "policy", "crudo-native", "dashboard", "lane board"]),
...
def find_source_omx(repo_root: Path) -> Path:
    ...
    for candidate in candidates:
        if (candidate / "plans" / "crudo-2-0-adaptive-plan.md").exists() and (candidate / "research").exists():
            return candidate
    raise FileNotFoundError("could not locate source .omx with plans/crudo-2-0-adaptive-plan.md and research/")
...
def stream_for(record: RoadmapRecord) -> str:
    ...
    if "phase 4" in combined or category_for(combined) == "task_policy":
        return "stream_4_crudo_first_execution"
...
def release_bucket_for(record: RoadmapRecord, status: str) -> str:
    ...
    if status == "context":
        return "context"
    if status == "rejected_not_crudo":
        return "rejected_not_crudo"
...
def status_for(record: RoadmapRecord) -> str:
    ...
    if record.source_type == "roadmap_heading" and (record.level <= 2 or title in STRUCTURAL_HEADINGS):
        ...
        if "pinpoint" not in title.lower() and not any(word in combined for word in ["gap", "routing"]):
            return "context"
    if any(word in combined for word in ["rejected_not_crudo", "not crudo", "outside crudo"]):
        return "rejected_not_crudo"
...
def deferral_for(record: RoadmapRecord, status: str) -> str:
    ...
    if status == "rejected_not_crudo":
        return "Rejected because the source describes clone-only breadth or behavior outside Crudo's machine-truth/crudo-native-harness identity."
...
def build_board(repo_root: Path) -> dict[str, Any]:
    ...
    plan_path = source_omx / "plans" / "crudo-2-0-adaptive-plan.md"
    ...
    latest_issues = load_json(research / "crudo-open-latest.json")
    all_issues = load_json(research / "crudo-issues.json")
    items.extend(issue_item(issue, "crudo-open-latest", "latest_open_issue", "2.x_intake") for issue in latest_issues)
    ...
    for issue in all_issues[:50]:
        ...
        if any(k in title_body for k in ["security", "windows", "install", "provider", "model", "session", "license", "zed", "spam", "plugin"]):
            items.append(issue_item(issue, "crudo-issues", "issue_theme", "beta_adoption"))
    ...
    board = {
        ...
        "sources": {
            ...
            "approved_plan": {
                "path": ".omx/plans/crudo-2-0-adaptive-plan.md",
                "sha256_prefix": sha256_prefix(plan_path),
            },
            "research": {
                "root": str(source_omx / "research"),
                "crudo_open_latest_count": len(latest_issues),
                "crudo_issues_count": len(all_issues),

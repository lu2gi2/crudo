#!/usr/bin/env bash
# dogfood-build.sh — Build crudo from current checkout and verify provenance.
#
...
#
#   CRUDO=$(bash scripts/dogfood-build.sh)
#
# Then dogfood with config isolation (avoids real user config bleeding in):
#
#   CRUDO_CONFIG_HOME=$(mktemp -d) $CRUDO plugins list --output-format json
#
set -euo pipefail
...
BINARY="$RUST_DIR/target/debug/crudo"
EXPECTED_SHA="$(git -C "$REPO_ROOT" rev-parse --short HEAD)"

echo "▶ Building crudo from $REPO_ROOT" >&2
...
echo "" >&2
echo "  export CRUDO=$BINARY" >&2
echo "" >&2
echo "  Dogfood with isolated config (no real user config on stderr):" >&2
echo "    CRUDO_ISOLATED=\$(mktemp -d)" >&2
echo "    trap 'rm -rf \"\$CRUDO_ISOLATED\"' EXIT" >&2
echo "    CRUDO_CONFIG_HOME=\ \ plugins list --output-format json" >&2
echo "" >&2
echo "  cargo run overhead: ~1s/invocation vs 7ms for pre-built binary." >&2
echo "  Prefer pre-built binary (\) for dogfood loops." >&2
echo "$BINARY"

...
class WorkspacePathScopeTests(unittest.TestCase):
    ...
    def test_shell_environment_expansion_is_validated(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            workspace.mkdir()
            outside.mkdir()
            previous = os.environ.get('CRUDO_SCOPE_OUTSIDE')
            os.environ['CRUDO_SCOPE_OUTSIDE'] = str(outside)
            try:
                self.assertEqual((f'{outside}/secret.txt',), extract_path_candidates('cat $CRUDO_SCOPE_OUTSIDE/secret.txt'))
                decision = WorkspacePathScope.from_root(workspace).validate_payload('cat $CRUDO_SCOPE_OUTSIDE/secret.txt')
            finally:
                if previous is None:
                    os.environ.pop('CRUDO_SCOPE_OUTSIDE', None)
                else:
                    os.environ['CRUDO_SCOPE_OUTSIDE'] = previous
